...
namespace Server.Engines.PartySystem
{
	public class Party : IParty
	{
		...
		public void Add( Mobile m )
		{
			...
			if ( mi == null )
			{
				...
				for ( int i = 0; i < m_Members.Count; ++i )
				{
                    ...
                    //If New Party Member is Ghost, Validate Its Sight
                    if (!f.Alive)
                    {
                        f.ValidateGhostVisibility();
                    }
			...
			else
			{
				for ( int i = 0; i < m_Members.Count; ++i )
				{
					if ( ((PartyMemberInfo)m_Members[i]).Mobile == m )
					{
                        ...
                        //If Party Member is Ghost, Validate Its Sight If Not Last Member of Party
                        if (!m.Alive)
                        {                            
		...
		public void Disband()
		{
            ...
            //For Each Old Party Membmer
            for (int i = 0; i < m_Members.Count; ++i)
            {
                //Validate Sight if Ghost
                if (!this[i].Mobile.Alive && m_Members.Count > 1)
                {
                    this[i].Mobile.ValidateGhostVisibility();
                }
            }           
