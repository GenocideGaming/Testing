...
namespace Server.Spells.Seventh
{
	public class ChainLightningSpell : MagerySpell
	{
        ...
        public override int BaseDamage { get { return 34; } }
        ...
        public void Target(IPoint3D p)
        {
            ...
            else if (SpellHelper.CheckTown(p, Caster) && CheckSequence())
            {
                ...
                //Creature
                if (creatureCaster != null && creatureCaster.NetState == null) // not a pseudoseer
                {
                ...
                else
                {
                    ...
                    if (targets.Count > 0)
                    {
                        ...
                        for (int i = 0; i < targets.Count; ++i)
                        {
                            ...
                            else if (Math.Abs(m.X - p.X) <= 1 && Math.Abs(m.Y - p.Y) <= 1)
                            {
                                // take 75% damage
                                damage *= FeatureList.SpellChanges.ChainLightningPercentDamageOneTileDistance;
                            }
                            else // 2 squares away
                            {
                                // take 50% damage
                                damage *= FeatureList.SpellChanges.ChainLightningPercentDamageTwoTileDistance;
                            }
