...
namespace Server.Items
{
	public abstract class BaseRanged : BaseMeleeWeapon
	{
		...
		{		
            BaseCreature creatureAttacker = attacker as BaseCreature;

            //Override To Make Stealth Attacks For Creatures Work Better
            if (creatureAttacker != null && creatureAttacker.StealthAttackActive)
            {
                bool canSwing = true;
                ...
                return GetDelay(attacker) + TimeSpan.FromSeconds(Math.Max(-FeatureList.SkillChanges.ArcheryShotTimeDexReduction * attacker.Dex + FeatureList.SkillChanges.ArcheryContinuousBaseShotTime, 1.25));
            }

            else if (DateTime.Now > (attacker.LastMoveTime + TimeSpan.FromSeconds(Math.Max(-FeatureList.SkillChanges.ArcheryShotTimeDexReduction * attacker.Dex + FeatureList.SkillChanges.ArcheryInitialBaseShotTime, 0.5)))) // 1.5ish s at 25 dex, 0.5s at 100
            {
                bool canSwing = true;
                ...
                if (canSwing && attacker.HarmfulCheck(defender))
                {
                    ...
                    if (OnFired(attacker, defender))
                    {
                        ...
                        if (pm != null && pm.ViolentSwingReady)
                        {
                        ...
                        if (pm != null)
                        {
                            pm.ProjectilesFiredThisSession++;
                        }
