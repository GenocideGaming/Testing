					...
					else
					{
						...
						for ( int j = 0; j < list.Count; ++j )
						{
							...
							if ( chain == null && !propertyChains.Contains( type ) )
								propertyChains[type] = chain = EntityProperties.GetPropertyInfoChain( e.Mobile, type, bc.Object, PropertyAccess.Read, ref failReason );

							if ( chain == null )
								continue;

							PropertyInfo endProp = EntityProperties.GetPropertyInfo( ref obj, chain, ref failReason );
