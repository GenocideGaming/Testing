	...
	public class Hiding
	{
		...
		public static TimeSpan OnUse( Mobile m )
		{
			...
			else 
			{
				if ( ok )
				{
                    foreach (AggressorInfo aggressor in m.Aggressors) // make it so they stop attacking even if they can see them with passive detect
                    {
                        if (aggressor.Attacker != null && aggressor.Attacker.Combatant == m)
                        {
                            aggressor.Attacker.Combatant = null;
                        }
                    }
                    foreach (AggressorInfo aggressed in m.Aggressed) // make it so they stop attacking even if they can see them with passive detect
                    {
                        if (aggressed.Defender != null && aggressed.Defender.Combatant == m)
                        {
                            aggressed.Defender.Combatant = null;
                        }
                    }
					m.Hidden = true;
					m.Warmode = false;
                    if (Spells.Sixth.InvisibilitySpell.HasTimer(m))
