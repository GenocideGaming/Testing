...
namespace Server.Items
{
	public class Bandage : Item, IDyable
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( from.InRange( GetWorldLocation(), Range ) )
			{
                ...
                if (player != null)
                    player.BandagesUsedThisSession++;
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
                ...
                if (targeted is BaseCreature && ((BaseCreature)targeted).Pseu_CanBeHealed == false)
                {
	...
	public class BandageContext
	{
		...
		public void EndHeal()
		{
			...
			{
                double slipsPenalty = m_Slips * FeatureList.Healing.BandageSlipImpact;

                //Cap Slip Impact
                if (slipsPenalty > FeatureList.Healing.BandageMaxSlipImpact)
                {
                    slipsPenalty = FeatureList.Healing.BandageMaxSlipImpact;
                }

                double successIncrement = FeatureList.Healing.RessurrectionSuccessAtGM / (100 - 80);
                double chance = ((healing - 80) * successIncrement) - slipsPenalty;

                //If Healer Has Valid Ress Skills
                if (checkSkills = (healing >= 80.0 && anatomy >= 80.0))
                {
                    //Check Successful   
                    if (chance > Utility.RandomDouble())
                    {
                        ...
                        else
                        {
                            ...
                            {
                                PlayerMobile healer = m_Healer as PlayerMobile;
                                if (healer != null)
                                    healer.ResurrectionsThisSession++;
                ...
                //Insufficient Skill to Attempt Ress
                else
                {                    
                    if (petPatient != null && petPatient.IsDeadPet)
                        healerNumber = 503256; // You fail to resurrect the creature.
			...
			else if ( m_Patient.Poisoned )
			{
                ...
                double slipsPenalty = m_Slips * FeatureList.Healing.BandageSlipImpact;
                double poisonDifficulty = m_Patient.Poison.Level * 0.1;
                ...
                double successIncrement = FeatureList.Healing.RessurrectionSuccessAtGM / (100 - 80);
                double chance = ((healing - 30.0) / 50.0) - poisonDifficulty - slipsPenalty;
               
                //If Anatomy and Healing Are Both Over 80
                if ((checkSkills = (healing >= 60.0 && anatomy >= 60.0)))
                {
                    //Check Successful
                    if (chance > Utility.RandomDouble())
                    {
                ...
                //Insufficient Skill to Attempt Cure
                else
                {
                    healerNumber = 1010060; // You have failed to cure your target!
                    patientNumber = -1;
            ...
            //Normal Healing Attempt
			else
			{
                double realHealingSkill = m_Healer.Skills[primarySkill].Base;
                if (realHealingSkill >= FeatureList.Healing.HealingRequiredForDefiniteSuccess
                    || ((realHealingSkill / FeatureList.Healing.HealingRequiredForDefiniteSuccess) > Utility.RandomDouble()))
                {
                    m_Healer.SendLocalizedMessage(500969); // You finish applying the bandages.
                    ...
                    int minHealed = (int)(Math.Round((healing / 3) + (anatomy / 5)));
                    int maxHealed = (int)(Math.Round((healing / 2) + (anatomy / 4)));

                    int randomResult = Utility.RandomMinMax(minHealed, maxHealed);

                    int amountHealed = (int)(Math.Round((double)randomResult) * (1 - slipsPenalty));

                    m_Patient.Heal((int)amountHealed, m_Healer, false);
                }
                else
                {
