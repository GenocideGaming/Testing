...
namespace Server.SkillHandlers
{
	class SpiritSpeak
	{
		...
		{
            if (m is BaseCreature && m.NetState != null)
            {
                m.SendMessage("You cannot spirit speak as a non-human!");
                return TimeSpan.FromSeconds(1.0);
            }
            m.RevealingAction();
            
            bool success = m.CheckSkill(SkillName.SpiritSpeak, 0, 100);
            ...
            pm.SpiritSpeakGhostSightExpiration = DateTime.Now + TimeSpan.FromSeconds(secs);

            //Will Scan Area for Ghosts and Force Visibility Checks for Each
            pm.CheckNearbyGhostVisibility();       
            
            t.Delay = TimeSpan.FromSeconds(secs); //30 seconds to 3 minutes
            t.Start();
			...
			{
                m.SendMessage("You fail to contact the netherworld, but ghosts may still detect your presence...");
		...
		private class SpiritSpeakTimer : Timer
		{
			...
			{
                //Will Scan Area for Ghosts and Force Visibility Checks for Each
                m_Owner.CheckNearbyGhostVisibility();    
