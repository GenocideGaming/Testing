	...
	public abstract class BaseFactionGuard : BaseCreature
	{
		...
		public override void OnSpeech( SpeechEventArgs e )
		{
			...
			if ( !e.Handled && InRange( from, ListenRange ) && from.Alive )
			{
				...
				{
					if ( m_Town != null && m_Faction.IsCommander( from ))
					{
						m_OrdersEnd = DateTime.Now + TimeSpan.FromSeconds( 10.0 );
						...
						if ( understood )
						{
							...
							for ( int i = 0; i < factions.Count; ++i )
							{
								...
								if ( faction != m_Faction && Insensitive.Contains( e.Speech, faction.Definition.TownshipName ) )
								{
        ...
        public override void OnRegionChange(Region Old, Region New)
        {
            ...
            //We can only exist in townships, or the hero home or dungeon regions, if we go anywhere else we teleport home
       
            if (New is CommonwealthRegion || New is MilitiaStronghold || New is PrincessDungeonRegion)
            {

            }
            else
            {
                Say("My commander will be furious if he sees my post unattended, I must return immediately!");
                MoveToWorld(m_Faction.OwningCommonwealth.HeroHomeRegion.GoLocation, Map.Felucca);
            }
        }
		...
		public BaseFactionGuard( string title ) : base( AIType.AI_Generic, FightMode.Closest, 10, 1, 0.2, 0.4 )
		{
