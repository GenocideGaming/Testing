...
namespace Server.Spells.Fourth
{
	public class ArchCureSpell : MagerySpell
	{
			...
			else if ( CheckSequence() )
			{
				...
				if ( targets.Count > 0 )
				{
					...
					for ( int i = 0; i < targets.Count; ++i )
					{
						...
						if ( poison != null )
						{
                            double cureEffectiveness = Caster.Skills[SkillName.Magery].Value * FeatureList.Poison.ArchCureEffectivenessMod;
                            ...
                            if (cureChance > Utility.Random(100) && m.CurePoison(Caster))
                                ++cured;
						}
		...
		public void TargetMobile( Mobile m )
		{
			...
			else if ( CheckSequence() )
			{
				...
				Effects.PlaySound( (IPoint3D)m.Location, Caster.Map, 0x299 );
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object o )
			{
				...
				if ( o is Mobile )
					m_Owner.TargetMobile( o as Mobile );
				else if ( p != null )
					m_Owner.Target( p );
			}
