...
namespace Server.Spells.Seventh
{
	public class GateTravelSpell : MagerySpell
	{
        ...
        {
            return TimeSpan.FromSeconds(FeatureList.SpellChanges.GateTravelCastTime);
        }
		...
		{
            if (Caster.Criminal && !Caster.Region.IsPartOf("Roache"))
			{
		...
		public void Effect( Point3D loc, Map map, bool checkMulti )
		{
            ...
            else if (Caster.Criminal && !Caster.Region.IsPartOf("Roache"))
			{
				Caster.SendLocalizedMessage( 1005561, "", 0x22 ); // Thou'rt a criminal and cannot escape so easily.
			}
			...
			{
                if
                    (Caster.Region.IsPartOf(typeof(Server.Regions.DungeonRegion))) // one way gate from a dungeon
                {
