...
namespace Server.Items
{
    ...
    public abstract class XmlQuest
    {
        ...
        public static void RegisterKill(Mobile m_killed, Mobile m_killer)
        {
            ...
            {
                BaseCreature bc = m_killer as BaseCreature;
                if (bc.Pseu_KeepKillCredit == false && !bc.Deleted && bc.NetState != null
                    && bc.NetState.Account != null && bc.NetState.Account.GetPseudoSeerLastCharacter() != null)
                {
                    // kill credit goes to the pseudoseer playermobile, if available
                    m_killer = bc.NetState.Account.GetPseudoSeerLastCharacter();
                }
                else if (bc.BardProvoked && bc.BardMaster != null)
                {
                    m_killer = bc.BardMaster;
                }
                else if (bc.Controlled && bc.ControlMaster != null)
                {
                    m_killer = bc.ControlMaster;
                }
            }
