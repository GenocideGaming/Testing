				...
				for ( int i = 0; i < m_Columns.Length; ++i )
				{
					if ( i > 0 && m_List.Count > 0 )
					{
						...
						if ( obj != null )
						{
							...
							PropertyInfo[] chain = EntityProperties.GetPropertyInfoChain( m_From, obj.GetType(), m_Columns[i], PropertyAccess.Read, ref failReason );
			...
			for ( int i = m_Page * EntriesPerPage, line = 0; line < EntriesPerPage && i < m_List.Count; ++i, ++line )
			{
				...
				else
				{
					for ( int j = 1; j < m_Columns.Length; ++j )
					{
						...
						PropertyInfo[] chain = EntityProperties.GetPropertyInfoChain( m_From, src.GetType(), m_Columns[j], PropertyAccess.Read, ref failReason );
						...
						{
							PropertyInfo p = EntityProperties.GetPropertyInfo( ref src, chain, ref failReason );
