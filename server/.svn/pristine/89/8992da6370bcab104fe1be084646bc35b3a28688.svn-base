	...
	public class FactionStone : BaseSystemController
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( from is PlayerMobile )
			{
				...
				{
                    PlayerCitizenshipState pcs = PlayerCitizenshipState.Find(from);

                    if (pcs != null && pcs.Commonwealth == m_Faction.OwningCommonwealth)
                        from.SendGump(new JoinStoneGump((PlayerMobile)from, m_Faction));
                    else
                        from.SendMessage("You must first join the township that owns this militia before joining.");
				}
			}
		}
