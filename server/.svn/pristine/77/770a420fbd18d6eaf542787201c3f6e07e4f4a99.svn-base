...
namespace Server.Regions
{
	public class GuardedRegion : BaseRegion
	{
		...
		public override void OnCriminalAction( Mobile m, bool message, bool isGuardCallingAction )
		{
			base.OnCriminalAction( m, message, isGuardCallingAction );

            if (!IsDisabled() && isGuardCallingAction)
                CheckGuardCandidate(m);
		...
		{
			if ( m is BaseGuard || !m.Alive || m.AccessLevel > AccessLevel.Player || m.Blessed || IsDisabled() || (m is BaseCreature && ((BaseCreature)m).GuardImmune == true))
				return false;
