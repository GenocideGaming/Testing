...
{
    [CorpseName("a Galven Guard marksmen corpse")]
    public class DungeonGuardRanged : BaseCreature
    {
        ...
        public DungeonGuardRanged(Serial serial)
            : base(serial)
        {
        }
        ...
        {
            private DungeonGuardRanged mDungeonGuardRanged;

            public TalkTimer(DungeonGuardRanged DungeonGuardRanged, TimeSpan delay, TimeSpan interval)
                : base(delay, interval)
            {
                mDungeonGuardRanged = DungeonGuardRanged;
            }

            protected override void OnTick()
            {
                base.OnTick();
                if (mDungeonGuardRanged.Deleted || mDungeonGuardRanged.NetState != null) { return; }
                //In Warmode
                if (mDungeonGuardRanged.Warmode)
                {
                     mDungeonGuardRanged.SayRandomCombatSpeech();
                }

                //Wandering
                else
                {
                    mDungeonGuardRanged.SayRandomEmote();
                }
            }
        }
    }
