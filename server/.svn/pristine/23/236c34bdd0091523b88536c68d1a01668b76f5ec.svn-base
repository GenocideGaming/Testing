...
namespace Server
{
	...
	public class Item : IEntity, IHued, IComparable<Item>, ISerializable, ISpawnable
	{
        ...
        private bool m_InStorage = false;
        public bool InStorage
        {
            get { return m_InStorage; }
            set { m_InStorage = value; }
        }
        ...
        private bool m_Breakable = true;
        [CommandProperty(AccessLevel.GameMaster)]
        public bool Breakable { get { return m_Breakable; } set { m_Breakable = value; } }

        private bool m_DoesNotDecay = false; // for making items that are movable but never decay (e.g. server birth rares)
        [CommandProperty(AccessLevel.GameMaster)]
        public bool DoesNotDecay { get { return m_DoesNotDecay; } set { m_DoesNotDecay = value; } }
		...
		public virtual bool Decays
		{
			get
			{
                return !m_DoesNotDecay && (Movable && Visible);
			}
		}
		...
		public virtual void Serialize( GenericWriter writer )
		{
            ...
            // version 12
            writer.Write((bool)m_InStorage);
            // version 11
            writer.Write((bool)m_GhostVisible);
            
            // version 10
            writer.Write((bool)Breakable);
            writer.Write((bool)DoesNotDecay);
		...
		public virtual void Deserialize( GenericReader reader )
		{
			...
			{
                case 12:
                    InStorage = reader.ReadBool();
                    goto case 11;
				case 11:
                    m_GhostVisible = reader.ReadBool();
                    goto case 10;
                case 10:
                    m_Breakable = reader.ReadBool();
                    m_DoesNotDecay = reader.ReadBool();
                    goto case 9;
                case 9:
				case 8:
				case 7:
		...
		{
            if (m_Parent is Container && ((Container)m_Parent).MovableContents && ((Container)m_Parent).Parent == null)
                return true;
            else if (m_Parent is Item)
		...
		public Item()
		{
			m_Serial = SerialFactory.NewItem;
			...
			WorldFactory.AddItem( this );

			Type ourType = this.GetType();
			m_TypeRef = WorldFactory.m_ItemTypes.IndexOf( ourType );

			if ( m_TypeRef == -1 )
			{
				WorldFactory.m_ItemTypes.Add( ourType );
				m_TypeRef = WorldFactory.m_ItemTypes.Count - 1;
			}
		}
