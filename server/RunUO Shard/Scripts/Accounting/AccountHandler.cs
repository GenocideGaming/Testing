...
namespace Server.Misc
{
	...
	public class AccountHandler
	{
		...
		private static bool AutoAccountCreation = !FeatureList.Database.Active;
		...
		private static CityInfo[] StartingCities = new CityInfo[]
			{
                new CityInfo( "Arden", "Arden Outpost", 1075072, 1278,   1287,    0, Map.Felucca  ),
                new CityInfo( "Seachild",  "Seachild Outpost",     1075073, 1867,  1411,    0, Map.Felucca ),
                new CityInfo( "Hearth",  "Hearth Outpost",   1075074, 1490,  1882,   0, Map.Felucca ),
			};
		...
		*/
		...
		{
            if (ip.Equals(IPAddress.Parse(FeatureList.Database.WebServerIP))) //if the webserver creates an account we always clear it
                return true;

			if ( !IPTable.ContainsKey( ip ) )
				return true;

            if (IPLimiter.Exemptions != null && IPLimiter.Exemptions.Contains(ip))
                return true;
        ...
        public static bool CreateAccount(string username, string password, string IP)
        {

            if (username.Length == 0 || password.Length == 0 || username.Trim().Length <= 0)
            {
                Console.WriteLine("Unsafe credentials, cannot create account.");
                return false;
            }
            ...
            if (!CanCreate(IPAddress.Parse(IP)))
            {
                Console.WriteLine("IP Address cannot create more accounts.");
                return false;
            }
            foreach (Account account in Accounts.GetAccounts())
            {
                ...
                {
                    Console.WriteLine("Non-unique username, cannot create account.");
                    return false;
                }
            }

            Console.WriteLine("Creating new account from the web server '{0}'", username);
