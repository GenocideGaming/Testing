...
namespace Server.Misc
{
	public class Titles
	{
		...
		public static void AwardFame( Mobile m, int offset, bool message )
		{
            ...
            //Calculate the citizenship bonus if there is one
            PlayerMobile pm = m as PlayerMobile;
            if (pm != null && pm.CitizenshipPlayerState != null)
            {
                CommonwealthResourceBonus crb = pm.CitizenshipPlayerState.GetResourceBonus(ResourceBonusTypes.Fame);
                if (crb != null)
                {
                    Console.WriteLine("{0} has recieved a fame bonus of {1} for being a citizen of {2}. ", m.Name, crb.HarvestBonus, pm.CitizenshipPlayerState.Commonwealth.Definition.TownName);
                    pm.SendMessage("You have received a fame bonus for being a citizen!");
                    offset += crb.HarvestBonus;
                }
            }
				...
				for ( int i = 0; i < m_FameEntries.Length; ++i )
				{
					...
					if ( fame <= fe.m_Fame || i == (m_FameEntries.Length - 1) )
					{
						...
						for ( int j = 0; j < karmaEntries.Length; ++j )
						{
							...
							{
								if(ke.m_Title == "Dread Lord" || ke.m_Title == "Dread Lady")
									title.AppendFormat( ke.m_Title, beheld.Name );
								else
