...
namespace Server.Misc
{
	public class CharacterCreation
	{
		...
		private static void FillBankbox( Mobile m )
		{
			...
			// Craft tools
			Backpack craftingToolsBP = new Backpack();
			craftingToolsBP.Name = "Crafting Tools";
			craftingToolsBP.Hue = Utility.RandomBlueHue();
			craftingToolsBP.AddItem( MakeNewbie( new Scissors() ) );
			craftingToolsBP.AddItem( MakeNewbie( new SewingKit( 1000 ) ) );
			craftingToolsBP.AddItem( MakeNewbie( new SmithHammer( 1000 ) ) );
			craftingToolsBP.AddItem( MakeNewbie( new FletcherTools( 1000 ) ) );
			craftingToolsBP.AddItem( MakeNewbie( new DovetailSaw( 1000 ) ) );
			craftingToolsBP.AddItem( MakeNewbie( new MortarPestle( 1000 ) ) );
			craftingToolsBP.AddItem( MakeNewbie( new ScribesPen( 1000 ) ) );
			craftingToolsBP.AddItem( MakeNewbie( new TinkerTools( 1000 ) ) );
			bank.DropItem(craftingToolsBP);

			// Resources
			Backpack resourcesBP = new Backpack();
			resourcesBP.Name = "Resources Backpack";
			resourcesBP.Hue = Utility.RandomGreenHue();
			resourcesBP.AddItem( MakeNewbie( new Feather( 1000 ) ) );
			resourcesBP.AddItem( MakeNewbie( new BoltOfCloth( 1000 ) ) );
			resourcesBP.AddItem( MakeNewbie( new BlankScroll( 1000 ) ) );
			resourcesBP.AddItem( MakeNewbie( new Hides( 1000 ) ) );
			resourcesBP.AddItem( MakeNewbie( new Bandage( 1000 ) ) );
			resourcesBP.AddItem( MakeNewbie( new Bottle( 1000 ) ) );
			resourcesBP.AddItem( MakeNewbie( new Log( 1000 ) ) );
			resourcesBP.AddItem( MakeNewbie( new Apple( 1000 ) ) );
			resourcesBP.AddItem( MakeNewbie( new IronIngot( 5000 ) ) );
			resourcesBP.AddItem( MakeNewbie( new DullCopperIngot( 5000 ) ) );
			resourcesBP.AddItem( MakeNewbie( new ShadowIronIngot( 5000 ) ) );
			resourcesBP.AddItem( MakeNewbie( new CopperIngot( 5000 ) ) );
			resourcesBP.AddItem( MakeNewbie( new BronzeIngot( 5000 ) ) );
			resourcesBP.AddItem( MakeNewbie( new GoldIngot( 5000 ) ) );
			resourcesBP.AddItem( MakeNewbie( new AgapiteIngot( 5000 ) ) );
			resourcesBP.AddItem( MakeNewbie( new VeriteIngot( 5000 ) ) );
			resourcesBP.AddItem( MakeNewbie( new ValoriteIngot( 5000 ) ) );
			bank.DropItem(resourcesBP);
			
			// Reagents
			Backpack reagentsBP = new Backpack();
			reagentsBP.Name = "Reagents Backpack";
			reagentsBP.Hue = Utility.RandomRedHue();
			reagentsBP.AddItem( MakeNewbie( new BlackPearl( 1000 ) ) );
			reagentsBP.AddItem( MakeNewbie( new Bloodmoss( 1000 ) ) );
			reagentsBP.AddItem( MakeNewbie( new Garlic( 1000 ) ) );
			reagentsBP.AddItem( MakeNewbie( new Ginseng( 1000 ) ) );
			reagentsBP.AddItem( MakeNewbie( new MandrakeRoot( 1000 ) ) );
			reagentsBP.AddItem( MakeNewbie( new Nightshade( 1000 ) ) );
			reagentsBP.AddItem( MakeNewbie( new SulfurousAsh( 1000 ) ) );
			reagentsBP.AddItem( MakeNewbie( new SpidersSilk( 1000 ) ) );
			bank.DropItem(reagentsBP);
		}
		...
		private static void EventSink_CharacterCreated( CharacterCreatedEventArgs args )
		{
            ...
            var city = args.City;
			var map = city.Map == Map.Felucca ? Map.Felucca : city.Map;
            ...
            //setup for citizenship
            GetCitizenship(newChar);

            //DatabaseController.AddCharacter(newChar);
...
/*
        private static readonly CityInfo MainCityInfo = new CityInfo("Galven", "Dong inn", 1310, 1386, 0, Map.Felucca);
...
*/
        private static void GetCitizenship(Mobile m)
        {
            ...
            for (int i = 0; i < acct.Length; i++)
            {
                if (acct[i] != null)
                {
                    ...
                    if (pm != null && m as PlayerMobile != pm)
                    {
                        //if another char is a citizen and NOT locked, then we join their commonwealth
                        if (pm.CitizenshipPlayerState != null && !pm.CitizenshipPlayerState.LockedToCommonwealth)
                        {
                            pm.CitizenshipPlayerState.Commonwealth.AddCitizen(m);
                            break;
                        }
                    }
                }
		...
		private static void SetSkills( Mobile m, SkillNameValue[] skills, int prof )
		{
			...
			for ( int i = 0; i < skills.Length; ++i )
			{
                ...
                if (snv.Name == SkillName.Fletching)
                    snv = new SkillNameValue(SkillName.Carpentry, snv.Value); // Converts fletching skill to carpentry
