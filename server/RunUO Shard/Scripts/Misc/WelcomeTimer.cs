...
namespace Server.Misc
{
	...
	public class WelcomeTimer : Timer
	{
		...
		private static string[] m_Messages = ( TestCenter.Enabled ?
			new string[]
				{
					"Welcome to the Rel Wis Beta Shard.  You are able to customize your character's stats and skills at anytime to anything you wish.  To see the commands to do this just say 'help'.",
		...
		protected override void OnTick()
		{
            ...
            if (m_State == m_Count)
            {
                if (UniqueNameChecker.CheckName(m_Mobile, m_Mobile.Name))
                {
                    RelwisGump.relwis_OnCommand(new CommandEventArgs(m_Mobile, "relwis", string.Empty, null));
                }
                Stop();
            }
		}
