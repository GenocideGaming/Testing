...
namespace Server.SkillHandlers
{
    ...
    public class TrackWhatGump : Gump
    {
            ...
            : base(20, 30)
        {
            ...
            //SkillCheck: Should Always Work
            if (from.CheckSkill(SkillName.Tracking, 0.0, 100.0))
            {
            }
    ...
    public class TrackWhoGump : Gump
    {
        ...
        public static void DisplayTo(bool success, Mobile from, int type)
        {
            ...
            int range = (int)(from.Skills[SkillName.Tracking].Value * FeatureList.SkillChanges.TrackingRangeScalar);
            
            List<Mobile> list = new List<Mobile>();

            foreach (Mobile m in from.GetMobilesInRange(range))
            {
                if (m != from && m.Alive && (m.AccessLevel == AccessLevel.Player || from.AccessLevel > m.AccessLevel) && check(m) && CheckDifficulty(from, m))
                    list.Add(m);
            }
        ...
        {
            bool withinEffectiveRange = false;

            double distance = from.GetDistanceToSqrt(m);    
    
            double range = from.Skills[SkillName.Tracking].Value * FeatureList.SkillChanges.TrackingRangeScalar;
            double detectHiddenStartingRange = range / FeatureList.SkillChanges.TrackingDetectHiddenStartingRangeDivider;
            double detectHiddenRange = detectHiddenStartingRange + ((range - detectHiddenStartingRange) * (from.Skills[SkillName.DetectHidden].Value / 100));
            
            if (!m.Hidden)
            {
                if (distance <= range)
                {
                    withinEffectiveRange = true;
                }
            }

            else
            {
                if (distance <= detectHiddenRange)
                {
                    withinEffectiveRange = true;
                }
            }

            return withinEffectiveRange;
        }
