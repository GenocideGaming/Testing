		...
		private class InternalTargetPoison : Target
		{
			...
			private class InternalTarget : Target
			{
				...
				private class InternalTimer : Timer
				{
					...
					{
                        double skill = m_From.Skills[SkillName.Poisoning].Value;
                        double difficulty = m_MinSkill;

                        double chance = ((skill - difficulty) * (100 / FeatureList.Poison.PoisoningDifficultyDivider)) / 100;
                        
                        //Skill Check
                        m_From.CheckTargetSkill(SkillName.Poisoning, m_Target, 0, 100);                        
                        ...
                        if (chance >= Utility.RandomDouble())
						{
							...
							else if ( m_Target is BaseWeapon )
							{
								...
								((BaseWeapon)m_Target).PoisonCharges = 10 - (m_Poison.Level * 2);
							}
