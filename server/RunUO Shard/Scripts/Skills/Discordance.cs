...
namespace Server.SkillHandlers
{
	public class Discordance
	{
		...
		private class DiscordanceInfo
		{
            ...
            public DateTime m_StrictEndTime;
		...
		private static void ProcessDiscordance( DiscordanceInfo info )
		{
            ...
            // Rel Por specific: made it so discordance doesn't last forever
            // if the bard is in range... now it always lasts a maximum of 1-2 minutes
            ...
            if (ends && info.m_Ending && info.m_EndTime < DateTime.Now || info.m_StrictEndTime < DateTime.Now)
			{
				...
				else if ( target is Mobile )
				{
					...
					else if ( !targ.Player )
					{
						...
						else if ( from.CheckTargetSkill( SkillName.Discordance, target, diff-25.0, diff+25.0 ) )
						{
                            ...
                            info.m_StrictEndTime = DateTime.Now + TimeSpan.FromSeconds(60 + 60*Utility.RandomDouble());
					...
					{
						m_Instrument.PlayInstrumentBadly( from );
                        from.SendMessage("They aren't listening.");
					}
				}
				else
				{
