...
namespace Server.SkillHandlers
{
	public class RemoveTrap
	{
		...
		public static TimeSpan OnUse( Mobile m )
		{
			...
			return TimeSpan.FromSeconds( 3.0 ); //delay before beign able to re-use a skill
		}
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				else if ( targeted is TrapableContainer )
				{
                    ...
                    double skill = from.Skills[SkillName.RemoveTrap].Value;
                    double difficulty = ((double)targ.TrapLevel) * 15;
                    double chance = (skill - difficulty) * ((100 / FeatureList.SkillChanges.RemoveTrapDifficultyDivider) / 100);
