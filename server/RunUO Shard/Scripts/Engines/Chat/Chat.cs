	...
	public class ChatSystem
	{
		...
		public static void ChatAction( NetState state, PacketReader pvSrc )
		{
			...
			try
			{
                ...
                // CUSTOM CODE for rel por--Chat b/w mobs with the same teamflags
                ...
                foreach (NetState nextstate in states)
                {
                    ...
                    {
                        nextstate.Mobile.SendMessage(101, "[Team " + from.TeamFlags + "] " + from.Name + ": " + param);
                    }
                    else if (AITeamList.CheckSameTeam(from, nextstate.Mobile))
                    {
