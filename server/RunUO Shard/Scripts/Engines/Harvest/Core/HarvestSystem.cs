			...
			if ( skillBase >= resource.ReqSkill && from.CheckSkill( def.Skill, resource.MinSkill, resource.MaxSkill ) )
			{
				...
				if ( type != null )
				{
					...
					else
					{
                        ...
                        //citizenship stuff - gives player extra resources without removing the extra from the bank
                        PlayerMobile pm = from as PlayerMobile;
                        CommonwealthResourceBonus citizenshipBonus = null;
                        if (pm != null && pm.CitizenshipPlayerState != null)
                        {
                            switch (def.Skill)
                            {
                                case SkillName.Fishing:
                                    citizenshipBonus = pm.CitizenshipPlayerState.GetResourceBonus(ResourceBonusTypes.Fishing);
                                    break;
                                case SkillName.Lumberjacking:
                                    citizenshipBonus = pm.CitizenshipPlayerState.GetResourceBonus(ResourceBonusTypes.Lumberjack);
                                    break;
                                case SkillName.Mining:
                                    citizenshipBonus = pm.CitizenshipPlayerState.GetResourceBonus(ResourceBonusTypes.Mining);
                                    break;
                            }
                        }

                        if (citizenshipBonus != null)
                        {
                            item.Amount += citizenshipBonus.HarvestBonus;
                            Console.WriteLine("A citizenship bonus of {0} to harvesting was given for skill {1}. ", citizenshipBonus.HarvestBonus, def.Skill.ToString());
                            from.SendMessage("Your harvesting has yielded bonus resources!");
                        }
