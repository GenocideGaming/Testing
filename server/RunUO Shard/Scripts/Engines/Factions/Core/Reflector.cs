...
namespace Server.Factions
{
	public class Reflector
	{
        ...
        private static List<ICommonwealth> mCommonwealths;
        public static  List<ICommonwealth> Commonwealths
        { 
            get
            {
                if (mCommonwealths == null)
                {
                    Reflector.ProcessTypes();
                }

              
                return mCommonwealths;
            }
		...
		private static void ProcessTypes()
		{
            ...
            mCommonwealths = new List<ICommonwealth>();

            if (CoreFactory.IsTest)
                return;
			...
			for ( int i = 0; i < asms.Length; ++i )
			{
				...
				for ( int j = 0; j < types.Length; ++j )
				{
                    ...
                    else if (type.IsSubclassOf(typeof(Commonwealth)))
                    {
                        Commonwealth commonwealth = Construct(type) as Commonwealth;

                        if (commonwealth != null)
                        {
                            Commonwealth.Commonwealths.Add(commonwealth);
