...
namespace Server.Factions
{
	...
	public abstract class BaseFactionTrap : BaseTrap
	{
		...
		public virtual TimeSpan DecayPeriod
		{
			get
			{
				return TimeSpan.FromDays(FeatureList.Militias.TrapExpirationTimeInDays);
			}
		}
		...
		public override void OnTrigger( Mobile from )
		{
			...
			int silverToAward = FeatureList.Militias.SilverRewardForKill;

			if ( silverToAward > 0 && m_Placer != null && m_Faction != null && !from.Alive)
			{
				PlayerState victimState = PlayerState.Find( from );

				if ( victimState != null && victimState.CanGiveSilverTo( m_Placer ))
				{
		...
		public virtual int IsValidLocation( Point3D p, Map m )
		{
            ...
            MilitiaNoTrapRegion noTrapRegion = Region.Find(p,m) as MilitiaNoTrapRegion;
            if (noTrapRegion != null)
                return 502956; // You cannot place a trap on that.
			...
			switch( AllowedPlacing )
			{
				...
				{
					MilitiaStronghold homeRegion = (MilitiaStronghold) Region.Find( p, m ).GetRegion( typeof( MilitiaStronghold ) );
                    PrincessDungeonRegion dungeonRegion = (PrincessDungeonRegion)Region.Find(p, m).GetRegion(typeof(PrincessDungeonRegion));

					if ( (homeRegion != null && homeRegion.Owner.Militia == m_Faction) || (dungeonRegion != null && dungeonRegion.Owner.Militia == m_Faction))
						return 0;
