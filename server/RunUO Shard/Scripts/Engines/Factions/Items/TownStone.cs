...
namespace Server.Factions
{
	public class TownStone : BaseSystemController
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if ( m_Town.Militia == null || ( from.AccessLevel < AccessLevel.GameMaster && faction != m_Town.Militia ) )
				from.SendLocalizedMessage( 1010332 ); // Your faction does not control this town
			else if ( !m_Town.Militia.IsCommander( from ) )
				from.SendLocalizedMessage( 1005242 ); // Only faction Leaders can use townstones
			else if ( FactionGump.Exists( from ) )
				from.SendLocalizedMessage( 1042160 ); // You already have a faction menu open.
			else if ( from is PlayerMobile )
				from.SendGump( new TownStoneGump( (PlayerMobile)from, m_Town.Militia, m_Town ) );
		}
