...
namespace Server.Engines.XmlSpawner2
{
    ...
    public class XmlAttach
    {
                    ...
                    if (attachtype != null && attachtype.IsSubclassOf(typeof(XmlAttachment)))
                    {
                        ...
                        for (int i = 0; i < list.Count; i++)
                        {
                            ...
                            if (alist != null)
                            {
                                foreach (XmlAttachment a in alist)
                                {
                                    PropertyInfo[] props = attachtype.GetProperties(BindingFlags.Static | BindingFlags.Instance | BindingFlags.Public);
        ...
        public static void CheckOnHit(Mobile m_hit, Mobile m_hitter)
        {
            ...
            {
                // check the killer
                ArrayList alist = XmlAttach.FindAttachments(m_hit);
                if (alist != null)
                {
                    foreach (XmlAttachment a in alist)
                    {
