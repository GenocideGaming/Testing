...
namespace Server.Mobiles
{
    ...
    public class BaseXmlSpawner
    {
        ...
        private static string CommandMobileName = "Galven Watch";
        ...
        public static void InitializeHash()
        {
            ...
            AddTypeKeyword("FX");
            AddTypeKeyword("FXTO");
            AddTypeKeyword("REWARD");
            AddTypeKeyword("RESETTEAMFLAGS");
            AddTypeKeyword("REMOVESUPPLIES");
            ...
            AddTypemodKeyword("POSSESS");
            AddTypemodKeyword("FX");
            AddTypemodKeyword("FXTO");
            ...
            AddValueKeyword("OFFX");
            AddValueKeyword("OFFY");
            AddValueKeyword("OFFZ");
        ...
        // -------------------------------------------------------------
        public static string InternalGetValue(object o, PropertyInfo p, int index)
        {
        ...
        public static bool ApplyObjectStringProperties(XmlSpawner spawner, string str, object o, Mobile trigmob, object refobject, out string status_str)
        {
            ...
            // if it's a damageableitem, we need to use the BaseCreature link; not much you can do with the item
            ...
            {
                o = ((DamageableItemMilitiaRestricted)o).Link;
            }
                ...
                if (arglist.Length > 1)
                {
                        ...
                        arglist[0].Length > 0 && !Char.IsUpper(arglist[0][0]))
                    {
                        ...
                        //else
                        {
                            ...
                            {
                                //need to to do escaping!
                                string lstr = singlearglist[1];
                                string escapedCharacters = "";
                                
                                lstr = ParseEscapedCharacters(lstr, out escapedCharacters, "@");
                                string escapedOutput = lstr; // need it unmodified at the end
                                int remainderIndex = -1;
                                for (int i = 1; i < lstr.Length; i++)
                                {
                                    // at least 3 extra remainder character behind the second @ symbol (3 is arbitrary)
                                    if (lstr[i] == '@' && i < lstr.Length - 3)
                                    {
                                        remainderIndex = i + 1;
                                ...
                                string result = SetPropertyValue(spawner, o, arglist[0], CompleteEscapedCharacters(lstr.Remove(0, 1), escapedCharacters));
                                ...
                                if (remainderIndex != -1)
                                {
                                    remainder = CompleteEscapedCharacters(escapedOutput.Substring(remainderIndex), escapedCharacters);
                                }
                                else
                                {
                                    remainder = null;
                    ...
                    else
                    {
                        if (IsValuemodKeyword(value_keywordargs[0]))
                        {
                            ...
                            {
                                // syntax is PLAYERSINRANGE,range[,alive]
                                int nplayers = 0;
                                int range = 0;
                                int alive = 0;
                                ...
                                if (value_keywordargs.Length > 2)
                                {
                                    try
                                    {
                                        alive = int.Parse(value_keywordargs[2]);
                                    }
                                    catch { }
                                }
                                ...
                                if (refobject is Item)
                                {
                                    ...
                                    {
                                        if (p.Player && p.AccessLevel == AccessLevel.Player && !p.Alive && alive == -1)
                                            nplayers++;
                                        if (p.Player && p.AccessLevel == AccessLevel.Player && p.Alive && alive == 1)
                                            nplayers++;
                                        if (p.Player && p.AccessLevel == AccessLevel.Player && alive == 0)
                                            nplayers++;
                                    }
                        ...
                        else if (IsTypemodKeyword(keywordargs[0]))
                        {
                            ...
                            else if (kw == typemodKeyword.FX)
                            {
                                int effect = -1;
                                int duration = 1;
                                // syntax is FX,itemid,,target
                                // try to get the effect argument
                                if (keywordargs.Length < 2)
                                {
                                    status_str = "Missing effect number";
                            ...
                            else if (kw == typemodKeyword.DAMAGE)
                            {
                                ...
                                ApplyDamage(arglist[0], o as Mobile, o, out status_str);
                            ...
                            else if (kw == typemodKeyword.POSSESS)
                            {
                                // syntax is POSSESS/serial|name
                                if (o is BaseCreature)
                                {
                                    ...
                                    if (arglist[1].StartsWith("0x"))
                                    {
                                        ...
                                        if (serial >= 0)
                                            testmob = World.FindEntity(serial);
                                    }
                                    else
                                    {
                                        int serial = -1;
                                        try
                                        {
                                            serial = Convert.ToInt32(arglist[1]);
                                        }
                                        catch
                                        {
                                            testmob = FindMobileByName(spawner, arglist[1], "playermobile");
                                        }
                                        if (serial >= 0)
                                            testmob = World.FindEntity(serial);
                                    }

                                    if (testmob is PlayerMobile)
                                    {
                                        CreaturePossession.ForcePossessCreature(null, testmob as PlayerMobile, o as BaseCreature);
                                    }
                                    else
                                    {
                                        status_str = "Possess fail: not a playermobile!";
                                    }
                                }
                                if (arglist.Length < 3) break;
                                remainder = arglist[2];
            ...
            if ((startc == '.') || (startc == '-') || (startc == '+') || (startc >= '0' && startc <= '9'))
            {
                ...
                if (startc == '"' || startc == '(')
                {
                    ...
                    else if (IsValueKeyword(pname))
                    {
                        ...
                        {
                            // syntax is PLAYERSINRANGE,range[,alive]
                            //alive can be -1 for dead players, 0 for alive and dead and 1 for only alive
                            ptype = typeof(Int32);
        ...
        public static void ExecuteActions(Mobile trigmob, object target, string actions)
        {
            if (actions == null || actions.Length <= 0) return;
            // execute any action associated with it
            // allow for multiple action strings on a single line separated by a semicolon
            string escapedCharacters = "";
            actions = BaseXmlSpawner.ParseEscapedCharacters(actions, out escapedCharacters, ";");
            string[] args = actions.Split(';');

            for (int j = 0; j < args.Length; j++)
            {
                string arg = args[j].Replace('§', ';');
                //arg = BaseXmlSpawner.ParseEscapedCharacters(actions, out escapedCharacters, ";"); // in case there are multiple levels
                if (arg.Trim().StartsWith("#CONDITION"))
                {
                    // test the specified condition string
                    // syntax is #CONDITION,proptest
                    // NOTE: You can have an "else" statement! this just involves another semicolon
                    //    in the successaction field, it would therefore look like this:
                    // #CONDITION,hits>10 $; orc $; skeleton
                    // reparse with only one arg after the comma, this allows property tests that use commas as well
                    //NOW: can nest conditionals with extra $ signs (e.g. $$; or $$$;)
                    arg = BaseXmlSpawner.ParseEscapedCharacters(arg, out escapedCharacters, ";");
                    string[] splitConditionArgs = arg.Split(';');
                    string[] ckeyvalueargs = BaseXmlSpawner.ParseCommaArgs(splitConditionArgs[0], 2);

                    string status_str;
                    if (splitConditionArgs.Length >= 2
                        || ckeyvalueargs.Length > 1)
                    {
                        // dont spawn if it fails the test
                        if (BaseXmlSpawner.CheckPropertyString(null, target, ckeyvalueargs[1], trigmob, out status_str))
                        {
                            ExecuteActions(trigmob, target, splitConditionArgs[1]);
                        }
                        else if (splitConditionArgs.Length > 2)
                        {
                            ExecuteActions(trigmob, target, splitConditionArgs[2]); // else action
                        }
                    }
                    else
                    {
                    ...
                    if (status_str != null) { trigmob.SendMessage(status_str); }
                }
                else
                {
                    ExecuteAction(trigmob, target, arg);
                }
            }
        ...
        public static void ExecuteAction(Mobile trigmob, object target, string action)
        {
            ...
            string substitutedtypeName = BaseXmlSpawner.ApplySubstitution(null, target, trigmob, action);
            ...
            {
                BaseXmlSpawner.SpawnTypeKeyword(target, TheSpawn, typeName, substitutedtypeName, true, trigmob, loc, map, out status_str);
            }
            else
            {
                ...
                try
                {
                    ...
                    if (o == null)
                    {
                        ...
                        if (o is Mobile)
                        {
                            ...
                            BaseXmlSpawner.ApplyObjectStringProperties(null, substitutedtypeName, m, trigmob, target, out status_str);
                        }
                        else
                            if (o is Item)
                            {
                                Item item = (Item)o;
                                BaseXmlSpawner.AddSpawnItem(null, target, TheSpawn, item, loc, map, trigmob, false, substitutedtypeName, out status_str);
                            }
                }
                catch { }
            }
        ...
        public static void ReportError(Mobile mob, string status_str)
        {
            if (status_str != null && mob != null && !mob.Deleted && mob is PlayerMobile && mob.AccessLevel > AccessLevel.Player)
            {
                mob.SendMessage(33, String.Format("error:{0}", status_str));
            }
        }

        public static bool CheckCondition(string condition, Mobile trigmob, object target)
        {
            ...
            if (condition != null && condition.Length > 0)
            {
                ...
                return BaseXmlSpawner.CheckPropertyString(null, target, condition, trigmob, out status_str);
            }
        ...
        public static bool CheckPropertyString(XmlSpawner spawner, object o, string testString, Mobile trigmob, out string status_str)
        {
            ...
            testString = BaseXmlSpawner.ApplySubstitution(spawner, o, trigmob, testString);
        ...
        public static string ApplySubstitution(XmlSpawner spawner, object o, Mobile trigmob, string typeName)
        {
            ...
            // first, escape characters with $ in front (like ${GETONTRIGMOB,x$} turns into $GETONTRIGMOB,x$, and later gets
            // put back into {GETONTRIGMOB,x} after the {} substitution is done
            string escapedCharacters = "";
            string remaining = ParseEscapedCharacters(typeName, out escapedCharacters, "{}");

            // start new stackbased parsing implementation here
            ...
            try
            {
                ...
                for (int i = 0; i < remaining.Length; i++)
                {
                    if (remaining[i] == '{')
                    {
                        stack.Push(remaining.Substring(startindex, i - startindex));
                        stack.Push("{");
                        startindex = i + 1;
                    }
                    else if (remaining[i] == '}')
                    {
                        string keypart = remaining.Substring(startindex, i - startindex);
                        while (stack.Count > 0)
                        {
                            string toAdd = stack.Pop();
                            if (toAdd == "{") break;
                            keypart = toAdd+keypart;
                        }
                        startindex = i + 1;
                ...
                startindex = 0;
                Console.WriteLine("xmlspawner parse error: " + remaining);
            }
            string output = "";
            while (stack.Count > 0)
            {
            ...
            if (remaining.Length > startindex)
            {
                output += remaining.Substring(startindex);
            }

            
            // now put back in the escaped characters
            return CompleteEscapedCharacters(output, escapedCharacters);

            // old method (can't be reached)
            ...
            // now put back in the escaped characters
            return CompleteEscapedCharacters(sb.ToString(), escapedCharacters);
        }
        ...
        public static string ParseEscapedCharacters(string str, out string escapedCharacters, string charactersToEscape)
        {
            if (str == null)
            {
                escapedCharacters = "";
            ...
            for (int i = 1; i < splitstring.Length; i++)
            {
                ...
                {
                    if (charactersToEscape.IndexOf(splitstring[i][0]) == -1)
                    {
                        // not trying to escape this time around--as in the following case:
                        // plate/ATTACH/<xmluse/condition/@${GETONTRIGMOB,hits$}$>10>
                        // where we want to only escape the {} characters in the ApplySubstitution function
                        // and only escape the > (and <, but not in this example) character in the ParseToMatchingParen function
                        sb.Append('$' + splitstring[i]);
                    }
                    else
                    {
                        sb.Append('§');
                        escapedCharacters += splitstring[i][0];
                        if (splitstring[i].Length > 1)
                        {
                            sb.Append(splitstring[i].Substring(1));
                        }
                    }
                }
                else
                {
                    // it's a lone $ character--tack it back on so it can be used in a nested situation
                    sb.Append('$');
                }
            }
            return sb.ToString();
        ...
        public static string CompleteEscapedCharacters(string str, string escapedCharacters)
        {
            string[] splitstring = str.Split('§');
            ...
            else if (splitstring.Length - 1 == escapedCharacters.Length)
            {
                System.Text.StringBuilder sb = new System.Text.StringBuilder();

                for (int i = 0; i < splitstring.Length - 1; i++)
                {
                    sb.Append(splitstring[i] + escapedCharacters[i]); // escaped characters length should always be i-1
                }
            ...
            {
                using (StreamWriter writer = new StreamWriter("xmlspawnerEscapeChar.txt", true))
                {
                    string message = "Was not able to escape correctly : " + str;
        ...
        {
            // first, escape characters with $ in front (like plate/ATTACH/<xmluse/condition/@GETONTRIGMOB,hits$<100>/hue/100 will evaluate to plate/ATTACH/<xmluse/condition/@GETONTRIGMOB,hits$100>/hue/100 and then the $ gets put back to < after the parsing
            string escapedCharacters = "";
            string remaining = ParseEscapedCharacters(str, out escapedCharacters, "<>");

            int nopen = 1;
            int nclose = 0;
            int splitpoint = remaining.Length;
            for (int i = 0; i < remaining.Length; i++)
            {
                // walk through the string until a matching close delimstr is found
                if (remaining[i] == opendelim) nopen++;
                if (remaining[i] == closedelim) nclose++;
            ...
            remaining = CompleteEscapedCharacters(remaining, escapedCharacters);
            args[0] = remaining.Substring(0, splitpoint);
            args[1] = "";
            if (splitpoint + 1 < remaining.Length)
            {
                args[1] = remaining.Substring(splitpoint + 1, remaining.Length - splitpoint - 1);
            }
        ...
        public static void ApplyDamage(string arglist, Mobile triggermob, object refobject, out string status_str)
        {
            status_str = null;
            Item refitem = null;
            Mobile refmob = null;
            ...
            // look for the other args. Syntax is DAMAGE,damagemin,damagemax[,range][,playeronly]
            // 
            string[] str = ParseString(arglist, 5, ",");
            bool playeronly = false;
            bool mobsonly = false;
            int range = -1;
            if (str.Length < 3)
            ...
            if (str.Length > 3)
            {
                ...
                try
                {
                    range = int.Parse(str[3]);
                }
                catch { status_str = "bad range arg in DAMAGE"; }
            }
            if (str.Length > 4)
            {
                if (str[4].ToLower() == "playeronly") playeronly = true;
                else if (str[4].ToLower() == "mobsonly") mobsonly = true;
            }
            ...
            try
            {
                damagemin = int.Parse(str[1]);
                damagemax = int.Parse(str[2]);
            }
                ...
                if (range >= 0 || (triggermob != null && !triggermob.Deleted))
                {
                    ...
                    if (range >= 0)
                    {
                        ...
                        if (rangelist != null)
                        {
                            foreach (Mobile p in rangelist)
                            {
                                if ((p is PlayerMobile && !mobsonly) || (p is BaseCreature && !playeronly))
                                    // AOS damage does not have any delay, which appears to cause freezing when it kills a mob!
                                    //AOS.Damage(p, p, damage, phys, fire, cold, pois, ener
                                    Spells.SpellHelper.Damage(TimeSpan.FromTicks(1), p, p, Utility.RandomMinMax(damagemin, damagemax));
                            }
                            rangelist.Free();
                        }
                    }
                    ...
                    {
                        // just apply it to the mob who triggered
                        //AOS.Damage(triggermob, triggermob, damage, phys, fire, cold, pois, ener);
                        if ((triggermob is PlayerMobile && !mobsonly) || (triggermob is BaseCreature && !playeronly))
                        {
                            Spells.SpellHelper.Damage(TimeSpan.FromTicks(1), triggermob, triggermob, Utility.RandomMinMax(damagemin, damagemax));
                        }
        ...
        public static void ApplyRewardToPlayers(string arglist, Mobile triggermob, object refobject, out string status_str)
        {
            status_str = null;
            Item refitem = null;
            Mobile refmob = null;
            ...
            // 
            string[] str = ParseString(arglist, 8, ",");
            string xmlvalueName = null;
            string scaletype = null;
            string eventname = null;
            int platinumAmount = 0;
            bool putInBank = false;
            ...
            if (str.Length < 5)
            {
                status_str = "missing reward args in REWARD";
            }
            else
            {
                xmlvalueName = str[1];
            ...
            {
                // get the range arg
                if (str[4].ToLower().Trim() == "bank" || str[4].ToLower().Trim() == "true")
                {
                    putInBank = true;
                }
            }
            ...
            if (str.Length > 6)
            {
                ...
                if (str.Length > 7)
                {
                    try 
                    {
                        probability = double.Parse(str[7]);
                    }
                    catch { status_str = "bad probability arg in REWARD"; }
                }
            }
            try
            {
                platinumAmount = int.Parse(str[3]);
            }
            catch { status_str = "bad platinumAmount arg in REWARD"; }

            // disburse the platinum reward and/or rare to the one with the highest score
            List<XmlValue> sortedValues = XmlAttachmentProcessing.Search(xmlvalueName);
            try
            {
                if (sortedValues.Count > 0 && sortedValues[0].Value != 0)
                {
                    ...
                    else if (scaletype == "exponential")
                    {
                        ...
                        if (maxValue > 0)
                        {
                            ...
                            foreach (XmlValue xmlValue in sortedValues)
                            {
                                ...
                                int awardedAmount = (int)Math.Round((1.0 - Math.Exp(-linearScaledValue * exponentialParameter)) * platinumAmount);
                                if (owner != null && !owner.Deleted)
                                {
                                    AwardPlatinum(putInBank, owner, awardedAmount, eventname);
                                    LogReward(xmlValue, awardedAmount, eventname);
                    ...
                    if (rare)
                    {
                         ...
                         if (owner != null && !owner.Deleted)
                         {
                             ...
                             {
                                 owner.SendMessage(38,"You were #1 in the " + eventname + " and got a special reward!");
                                 owner.AddToBackpack(new IOU());
                                 LogReward(sortedValues[0], 9999, eventname); // temporary approach
                             }
                             else
                             {
                                 owner.SendMessage(38, "Although you were #1 in the " + eventname + ", you didn't get a special reward this time. So sorry!");
                             }
                         }
                    }
                }
        ...
        private static void AwardPlatinum(bool putInBank, Mobile mob, int platinumAmount, string eventname)
        {
            if (platinumAmount == 0)
            {
                mob.SendMessage(38, "Sorry, but you did not acquire enough points to gain any platinum for the " + (eventname != null ? eventname : "event") + ".");
                return;
            }
            ...
            {
                mob.AddToBackpack(new Platinum(platinumAmount));
            }
            mob.SendMessage(38, "You have been awarded " + platinumAmount + " platinum coin(s) for your participation in the " + (eventname != null ? eventname : "event") + "! " + (putInBank ? "It has been placed into your bank." : ""));
        }

        private static void LogReward(XmlValue value, int platinumAmount, string eventname)
        {
            if (value == null || value.Owner == null || !(value.Owner is Mobile)) return;
            Mobile owner = value.Owner as Mobile;
    ...
    string propertyString, out string status_str)
            ...
            List<XmlSpawner.SpawnPositionInfo> spawnpositioning, string propertyString, bool smartspawn, out string status_str)
        {
            ...
            if (item is DamageableItem && ((DamageableItem)item).Link == null)
            {
                ((DamageableItem)item).ProvideEntity();
            }
            else if (item is DamageableItemMilitiaRestricted && ((DamageableItemMilitiaRestricted)item).Link == null)
            {
                ((DamageableItemMilitiaRestricted)item).ProvideEntity();
            }
            ...
            List<XmlSpawner.SpawnPositionInfo> spawnpositioning, Mobile triggermob, Point3D location, Map map, XmlGumpCallback gumpcallback, out string status_str)
        {
            ...
            if (IsTypeKeyword(typeName))
            {
                ...
                switch (kw)
                {
                    case typeKeyword.SET:
                        {
                            ...
                            if (keywordargs.Length > 1)
                            {
                                ...
                                {
                                    int serial = -1; // check for hexadecimal form
                                    try
                                    {
                                ...
                                {
                                    int serial = -1; // check for decimal form
                                    try
                                    {
                                        serial = Convert.ToInt32(keywordargs[1], 10);
                                    }
                                    catch { }
                                    if (serial >= 0)
                                        setitem = World.FindEntity(serial);
                    ...
                    case typeKeyword.REMOVESUPPLIES:
                        {
                            List<Item> toDelete = new List<Item>();
                            foreach (Item item in World.Items.Values)
                            {
                                if (item.Name == "eventsupply")
                                {
                                    toDelete.Add(item);
                                }
                            }
                    ...
                    case typeKeyword.RESETTEAMFLAGS:
                        {
                    ...
                    case typeKeyword.REWARD:
                        {
                            ...
                            if (arglist.Length > 0)
                            {
                                ApplyRewardToPlayers(arglist[0], triggermob, invoker, out status_str);
                                if (status_str != null)
                                {
                                    return false;
                                }
                    ...
                    case typeKeyword.EFFECT:
                        {
                            ...
                            if (arglist.Length > 0)
                            {
                                string[] keywordargs = ParseString(arglist[0], 8, ",");
                                int effect = -1;
                                int duration = 1;
                                ...
                                if (keywordargs.Length > 6)
                                {
                                    try
                                    {
                                        range = int.Parse(keywordargs[6]);
                                    }
                                    catch { status_str += " Improper range"; }
                                }
                                int zoffset = -1;
                                if (keywordargs.Length > 7)
                                {
                                    try
                                    {
                                        zoffset = int.Parse(keywordargs[7]);
                                    }
                                    catch { status_str += " Improper z-offset"; }
                                }
                                if (status_str != null)
                                {
                                    return false;
                                ...
                                if (effect >= 0)
                                {
                                    if (range == -1)
                                    {
                                        if (zoffset != -1) eloc.Z += zoffset;
                                        Effects.SendLocationEffect(eloc, emap, effect, duration);
                                    }
                                    else
                                    {
                                        if (zoffset != -1) eloc.Z += zoffset;
                                        for (int i = eloc.X - range; i <= eloc.X + range; i++)
                                        {
                                            for (int j = eloc.Y - range; j <= eloc.Y + range; j++)
                                            {
                                                Effects.SendLocationEffect(new Point3D(i,j,eloc.Z), emap, effect, duration);
                                            }
                                        }
                                    }
                    ...
                    case typeKeyword.DAMAGE:
                        {
                            ...
                            if (arglist.Length > 0)
                            {
                                ApplyDamage(arglist[0], triggermob, invoker, out status_str);
                                if (status_str != null)
                                {
                                    return false;
                                }
