                    ...
                    foreach (DataRow dr in ds.Tables[SpawnTablePointName].Rows)
                    {
                        ...
                        if ((SpawnerPrefix == null) || (SpawnerPrefix.Length == 0) || (SpawnName.StartsWith(SpawnerPrefix) == true))
                        {
                            ...
                            if (!bad_spawner)
                            {
                                ...
                                // reset the spawner
                                TheSpawn.Reset();
                                //TheSpawn.Running = SpawnIsRunning; // was moved down below b/c ResetNextSpawnTimes sets Running to true
                    ...
                    else
                    {
                        switch (positioning)
                        {
                            ...
                            case SpawnPositionType.RelXY:
                                ...
                                break;
                            case SpawnPositionType.DeltaLocation:
                                // Still checks whether it "Map.CanFit", which seems silly when location is specified using #DX,x,y[,z].
                                x = this.X + xinc;
                                y = this.Y + yinc;
                                defaultZ = this.Z + zinc;
                                if (!requiresurface) return new Point3D(xinc, yinc, zinc); // i added this to fix that problem
                                break;
                            case SpawnPositionType.Location:
                                // Still checks whether it "Map.CanFit", which seems silly when location is specified using #DX,x,y[,z].
                                x = xinc;
