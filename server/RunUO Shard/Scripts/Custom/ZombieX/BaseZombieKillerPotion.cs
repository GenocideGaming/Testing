...
namespace Server.Items
{
    public abstract class BaseZombieKillerPotion : BasePotion
    {
        ...
        {
            private readonly BaseZombieKillerPotion _potion;

            public BaseZombieKillerPotion Potion
            {
                get { return _potion; }
            }
        ...
        public void Explode(Mobile from, bool direct, Point3D loc, Map map)
        {
            ...
            for (int i = 0; i < toExplode.Count; ++i)
            {
                ...
                if (o is Mobile)
                {
                    ...
                    else if (from == null || (SpellHelper.ValidIndirectTarget(from, m) && from.CanBeHarmful(m, false)))
                    {
                        if (from != null)
                            from.DoHarmful(m);
