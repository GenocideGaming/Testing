...
{
    [CorpseName("the corpse of Kvothe")]
    public class Kvothe : BaseCreature
    {
        // 02-08-2021: Added healing time delay.
            ...
            : base(AIType.AI_Mage, FightMode.Aggressor, 10, 1, 0.2, 0.4 )
        {
            ...
            SetWearable(new LongPants(0x66D));
            SetWearable(new ThighBoots());
            SetWearable(new TricorneHat(0x1));
            SetWearable(new BodySash(0x66D));
            */
            ...
            this.FacialHairItemID = 0x204C; // Long Beard
            this.FacialHairHue = 0x485;
        ...
        // 02-08-2021: Added SmallDragonBoatDeed drop on death.
        public override void OnDeath(Container c)
        {
            base.OnDeath(c);

            c.DropItem(new SmallDragonBoatDeed());
            c.DropItem(new PowerScroll(SkillName.Tailoring, 105));
        }
        ...
        // 02-08-2021: Added timed call to HealSelf().
        public override void OnThink()
        {
            ...
            if (m_NextHeal <= DateTime.UtcNow && 0.05 > Utility.RandomDouble())
            {
                HealSelf();
            }
        }
        ...
        public override void OnGotMeleeAttack(Mobile attacker)
        {
            ...
            if (0.05 >= Utility.RandomDouble())
                SpawnUndeadPirates(attacker);
        }

        public override void AlterDamageScalarFrom(Mobile caster, ref double scalar)
        {
            if (0.05 >= Utility.RandomDouble())
                SpawnUndeadPirates(caster);
        }
        ...
        */
        ...
        // 02-08-2021: Added healing method.
        public void HealSelf()
        {
            ...
            int healAmount = (int)(this.HitsMax * 0.05);
            if (this.Hits + healAmount > this.HitsMax)
            {
                healAmount = this.HitsMax - this.Hits;
            }

            this.Hits += healAmount;
            this.FixedParticles(0x376A, 9, 32, 5030, EffectLayer.Waist);

            m_NextHeal = DateTime.UtcNow + TimeSpan.FromSeconds(30 + Utility.RandomMinMax(0, 30));
        }

        // 02-08-2021: Added number of spawned creatures limit check and creature unsummon timer.
        public void SpawnUndeadPirates(Mobile target)
        {
            Map map = Map;
            ...
            int currentUndeadPirates = 0;
            IPooledEnumerable eable = GetMobilesInRange(15);

            foreach (Mobile mob in eable)
            {
                if (mob is UndeadPirate)
                {
                    currentUndeadPirates++;
                }
            }

            eable.Free();

            if (currentUndeadPirates >= 15)
            {
                return;
            }

            int newUndeadPirates = Utility.RandomMinMax(2, 3);

            if (currentUndeadPirates + newUndeadPirates > 15)
            {
                newUndeadPirates = 15 - currentUndeadPirates;
            }

            for (int i = 0; i < newUndeadPirates; ++i)
            {
                UndeadPirate UndeadPirate = new UndeadPirate();

                UndeadPirate.Team = Team;
                UndeadPirate.FightMode = FightMode.Closest;
                ...
                UndeadPirate.MoveToWorld(loc, map);
                UndeadPirate.Combatant = target;

                new Server.Spells.UnsummonTimer(this, UndeadPirate, TimeSpan.FromMinutes(5)).Start();
            }
        }
