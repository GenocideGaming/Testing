...
namespace System.CustomizableVendor
{
    ...
    public class ManageItemsGump : JewlRewardGump
    {
        ...
        protected override void AddEntryControl(Reward r)
        {
            ...
            AddButton(470, (PosY + 55), 4011, 4012, m_ButtonNum, GumpButtonType.Reply, PageNum);
            AddLabel(508, (PosY + 57), 1882, @"Options");
            //even numbers
            AddButton(470, (PosY + 29), 4020, 4021, ++m_ButtonNum, GumpButtonType.Reply, PageNum);
            AddLabel(509, (PosY + 31), 1882, @"Delete");
            PosY += 102;

            //add new page every four entries
            if (EntryNum % 4 == 0)
            {
                AddButton(546, 550, 9903, 9904, -1, GumpButtonType.Page, PageNum + 1);

                AddPage(++PageNum);

                AddButton(221, 550, 9909, 9910, -1, GumpButtonType.Page, PageNum - 1);
    ...
    public class JewlRewardGump : Gump, IRewardVendorGump
    {
        ...
        protected virtual void CreateBackgroundControl()
        {
            ...
            AddLabel(348, 45, 2124, m_Vendor.GetName());
            AddImage(210, 52, 9818);
            AddImage(534, 52, 9818);
            AddLabel(348, 68, 2125, @"You have:");     //(404, 119, 2125,

            if(m_Vendor.Payment.PayName == "Player PlatinumCoins")
            {
                AddItem(300, 75, 3820, 2139);    //(348, 126,
                AddLabel(350, 85, 2125, (m_CurrencyAmnt + " Platinum Coins"));    //(404, 137, 2125,
            }
            else
            {
                AddItem(300, 75, m_Vendor.Payment.PayID, m_Vendor.Payment.CurrHue);    //(348, 126,
                AddLabel(350, 85, 2125, (m_CurrencyAmnt + " " + m_Vendor.Payment.PayName));    //(404, 137, 2125,
            }
    ...
    /*
        CUSTOMIZED MOBILE REWARD VENDOR

        The old Mobile Reward Vendors no longer work. These still give the benefit/appearance of a mobile but are tied to 
        a stone reward vendor to handle the display & buying of items. 

        The stone reward vendor is invisilbe, it's box that holds items is still visible via Ctrl + Shift. You CAN make these
        not visible, but doing so will make it so the player cannot view the item in the display chest before buying it.
    */
    public class MobileStoneRewardVendor : BaseCreature
    {
        private StoneRewardVendor m_stonevendor;

        [Constructable]
        public MobileStoneRewardVendor() : base( AIType.AI_Vendor, FightMode.None, 2, 1, 0.5, 2 )
        {
            Name = "Mobile Reward Vendor";
            Body = 0x190;
            ...
            Timer.DelayCall(TimeSpan.Zero, CreateStoneRewardVendor);
        }
        ...
        public override void AddCustomContextEntries(Mobile from, List<ContextMenuEntry> list)
        {
            ...
            list.Add(new MobileVendorBuy(@from, this));
        }
        ...
        protected override void OnLocationChange(Point3D oldLocation)
        {
            ...
            if(m_stonevendor != null && !m_stonevendor.Deleted)
            {
                m_stonevendor.Location = this.Location;
                m_stonevendor.Map = this.Map;
            }
        }
        ...
        public override void OnDelete()
        {
            if(m_stonevendor != null && !m_stonevendor.Deleted)
            {
                m_stonevendor.Delete();
            }

            base.OnDelete();
        }

        private void CreateStoneRewardVendor()
        {
            if(this.Map != Map.Internal)
            {
                StoneRewardVendor stonevendor = new StoneRewardVendor();
                m_stonevendor = stonevendor;
                stonevendor.ItemID = 1310;
                stonevendor.Visible = false;
                stonevendor.MoveToWorld(this.Location, this.Map);
            }
        }
		...
		{
            if(m_stonevendor != null && !m_stonevendor.Deleted)
                m_stonevendor.OnDoubleClick(from);
		}

        public MobileStoneRewardVendor(Serial serial)
            : base(serial)
        {
        }
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            writer.WriteItem(m_stonevendor);
        }
        ...
        public override void Deserialize(GenericReader reader)
        {
            ...
            switch (version)
            {
                case 0:
                {
                    m_stonevendor = (StoneRewardVendor)reader.ReadItem();
    ...
    public class MobileVendorBuy : ContextMenuEntry
    {
        private readonly MobileStoneRewardVendor m_Vendor;
        private readonly Mobile m_Mobile;

        public MobileVendorBuy(Mobile from, MobileStoneRewardVendor vendor)
            : base(6103, 8)
        {
            m_Vendor = vendor;
            m_Mobile = from;
        ...
        public override void OnClick()
        {
            try
            {
                m_Vendor.OnDoubleClick(m_Mobile);
            }
            catch
            {
                m_Mobile.SendMessage("The current display is invalid.");
    ...
    ///
    /// These no longer work, please use the MobileStoneRewardVendor instead
    ///
    public class MobileRewardVendor : BaseVendor, IRewardVendor
    {
        ...
        ///
        /// These no longer work, please use the MobileStoneRewardVendor instead
        /// Removed Constructor to prevent accidentally adding these to the world
        ///
