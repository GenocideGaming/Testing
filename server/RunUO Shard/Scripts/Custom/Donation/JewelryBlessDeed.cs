...
{
	public class JewelBlessTarget : Target // Create our targeting class (which we derive from the base target class)
	{
		private JewelBlessDeed m_Deed;

		public JewelBlessTarget( JewelBlessDeed deed ) : base( 1, false, TargetFlags.None )
		{
			m_Deed = deed;
		}
		...
		protected override void OnTarget( Mobile from, object target ) // Override the protected OnTarget() for our feature
		{
			...
			if (target is BaseJewel)
			{
				...
				if ( item is IArcaneEquip )
				{
					...
					if ( eq.IsArcane )
					{
						from.SendLocalizedMessage( 1005019 ); // This bless deed is for Jewels only.
						return;
					}
				}
	...
	public class JewelBlessDeed : Item // Create the item class which is derived from the base item class
	{
		...
		[Constructable]
		public JewelBlessDeed() : base( 0x14F0 )
		{
			Weight = 1.0;
			LootType = LootType.Blessed;
		}

		public JewelBlessDeed( Serial serial ) : base( serial )
		{
		}
		...
		public override void OnDoubleClick( Mobile from ) // Override double click of the deed to call our target
		{
			...
			else
			{
				...
				from.Target = new JewelBlessTarget( this ); // Call our target
			 }
		}	
	}
}