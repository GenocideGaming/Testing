...
namespace Server.Mobiles
{
	...
	public abstract class BaseVendor : BaseCreature, IVendor
	{
        ...
        private ICommonwealth mOwningCommonwealth;
        ...
        public virtual bool ConsumesSilver { get { return false; } }
        ...
        public TimeSpan m_RestockDelay = TimeSpan.Zero;
        [CommandProperty(AccessLevel.GameMaster)]
		public virtual TimeSpan RestockDelay
		{
			get
			{
                if (m_RestockDelay == TimeSpan.Zero)
                {
                    Region region = Region.Find(Location, Map);
                    CommonwealthRegion township = region as CommonwealthRegion;
                    if (township != null)
                    {
                        if (township.MyCommonwealth.State.VendorLevel == VendorLevels.DiligentMerchants)
        ...
        private void SetCommonwealthRegion()
        {
            CommonwealthRegion townshipRegion = Region as CommonwealthRegion;
            if (townshipRegion != null)
                mOwningCommonwealth = townshipRegion.MyCommonwealth;
            else
                mOwningCommonwealth = null;
                
        }
        private bool InMyCommonwealth(Mobile from)
        {
            if (mOwningCommonwealth == null)
                return false;

            ICommonwealth playerCommonwealth = Commonwealth.Find(from);
            if (playerCommonwealth == null)
                return false;

            if (mOwningCommonwealth == playerCommonwealth)
                return true;
		...
		public virtual void VendorBuy( Mobile from )
		{
            ...
            bool isInMyCommonwealth = InMyCommonwealth(from);
            if (isInMyCommonwealth)
                from.SendMessage("You have recieved a discount due to citizenship!");
			...
			for ( int idx = 0; idx < buyInfo.Length; idx++ )
			{
                ...
                if (isInMyCommonwealth)
                    price = (int)Math.Floor(price * FeatureList.Citizenship.BuyDiscount);
        ...
        private void ProcessSinglePurchase( BuyItemResponse buy, IBuyItemInfo bii, List<BuyItemResponse> validBuy, bool isInMyCommonwealth, ref int controlSlots, ref bool fullPurchase, ref int totalCost )
		{
            ...
            if (isInMyCommonwealth)
            {
                int itemCost = (int)Math.Floor(bii.Price * FeatureList.Citizenship.BuyDiscount) * amount;
                totalCost += itemCost > 1 ? itemCost : 1;
            }
            else
        ...
        public virtual bool OnBuyItems( Mobile buyer, List<BuyItemResponse> list )
		{
			...
			foreach ( BuyItemResponse buy in list )
			{
				...
				if ( ser.IsItem )
				{
					...
					if ( gbi != null )
					{
						ProcessSinglePurchase( buy, gbi, validBuy, InMyCommonwealth(buyer), ref controlSlots, ref fullPurchase, ref totalCost );
					}
					else if ( item != this.BuyPack && item.IsChildOf( this.BuyPack ) )
					{
						...
						foreach ( IShopSellInfo ssi in info )
						{
							if ( ssi.IsSellable( item ) )
							{
								...
								{
                                    if (InMyCommonwealth(buyer))
                                    {
                                        int itemCost = (int)Math.Floor(ssi.GetBuyPriceFor(item) * FeatureList.Citizenship.BuyDiscount) * amount;
			...
			if ( !bought && cont != null )
			{
                Item[] goldStacks = cont.FindItemsByType(typeof(Gold));

                int goldAmount = 0;

                foreach (Item goldStack in goldStacks)
                {
                    goldAmount += goldStack.Amount;
                }

                if (goldStacks != null && goldAmount >= totalCost)
                {
                    bought = true;
            ...
            PlayerMobile player = buyer as PlayerMobile;
            if (player != null)
                player.GoldSpentThisSession += totalCost;
        ...
        private class BulkOrderInfoEntry : ContextMenuEntry
        {
            ...
            public override void OnClick()
            {
                if (m_Vendor.SupportsBulkOrders(m_From))
                {
                    ...
                    if (((Core.T2A) ? totalMinutes == 0 : totalHours == 0))
                    {
