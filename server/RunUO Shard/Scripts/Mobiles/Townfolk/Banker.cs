...
namespace Server.Mobiles
{
	public class Banker : BaseVendor
	{
		...
		public static int GetSilverBalance( Mobile from )
		{
			int balance = 0;
			...
			if ( bank != null )
			{
				Item[] silver = bank.FindItemsByType( typeof( Silver ) );

				for ( int i = 0; i < silver.Length; ++i )
					balance += silver[i].Amount;
			}
				...
				for ( int i = 0; i < e.Keywords.Length; ++i )
				{
					...
					switch ( keyword )
					{
						case 0x0000: // *withdraw*
						{
							...
							if ( e.Mobile.Criminal && this.Region.Name != "Roache" )
							{
