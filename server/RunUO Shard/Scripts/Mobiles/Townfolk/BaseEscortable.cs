...
namespace Server.Mobiles
{
    ...
    {
        public virtual int GoldAmount()
        {
            return (Utility.RandomMinMax(FeatureList.Escortables.PaymentMin, FeatureList.Escortables.PaymentMax));
        }

        public string IAmLooking = "I am looking to go to {0}, will you take me?";

        public virtual string TakeMe()
        {
            return IAmLooking;
        }

        public string Leading = "Lead on! Payment will be made when we arrive in {0}.";

        public virtual string LeadOn()
        {
            return Leading;
        }

        public string Thanking = "We have arrived! I thank thee! I have no further need of thy services. Here is thy pay.";

        public virtual string ThankYou()
        {
            return Thanking;
        }
        ...
        public virtual bool Escortable { get { return true; } }
        public override bool ReturnsHome { get { return false; } }

        public override bool Commandable { get { return false; } }
        public bool UnstoppableDeletion = false;
        ...
        {
            "Pedran", "Lillano", "Vermell",
            "Bowan", "Calor", "Galven",
            "Arbor", "Trinsic", "New Skara",
            "Yew", "Vesper",
        };
        ...
        public virtual bool SayDestinationTo(Mobile m)
        {
            ...
            if (escorter == null)
            {
                Say(TakeMe(), dest.Name);
                return true;
            }
            else if (escorter == m)
            {
                Say(LeadOn(), dest.Name);
                return true;
            }
        ...
        private static TimeSpan m_EscortDelay = TimeSpan.FromMinutes(FeatureList.Escortables.BlockTimeInMinutes);
        ...
        public virtual bool AcceptEscorter(Mobile m)
        {
            ...
            else if (SetControlMaster(m))
            {
                ...
                BeginDelete(FeatureList.Escortables.DeleteTimeAfterAcceptance);
                Say(LeadOn(), dest.Name);
        ...
        protected DateTime m_LastSeenEscorter;
        ...
        public virtual void BeginDelete(double DeleteTime)
        {
            if (m_DeleteTimer != null)
                m_DeleteTimer.Stop();

            m_DeleteTime = DateTime.Now + TimeSpan.FromMinutes(DeleteTime);
        ...
        public virtual bool CheckAtDestination()
        {
            ...
            {
                Say(ThankYou(), escorter.Name); // We have arrived! I thank thee, ~1_PLAYER_NAME~! I have no further need of thy services. Here is thy pay.
                ...
                BeginDelete(FeatureList.Escortables.DeleteTimeAfterCompletion);
    ...
    public class EscortDestinationInfo
    {
        ...
        public static void LoadTable()
        {
            ...
            foreach (Region r in list)
            {
                ...
                if (r is Regions.DungeonRegion || r is Regions.TownRegion || r is Regions.CommonwealthRegion || r is Regions.GuardedRegion)
                    m_Table[r.Name] = new EscortDestinationInfo(r.Name, r);
            }

            // Add custom Britain Dockhouse region
            m_Table["Britain Dockhouse"] = new EscortDestinationInfo("Britain Dockhouse", new Region("Britain Dockhouse", Map.Felucca, 0, new Rectangle2D(1437, 1761, 10, 10)));
        }
    ...
    public class AbandonEscortEntry : ContextMenuEntry
    {
        ...
        public override void OnClick()
        {
            if (m_Mobile.Escortable)
                m_Mobile.Delete(); // OSI just seems to delete instantly
            else
            {
                // Bit hacky, don't care
                TownHero hero = m_Mobile as TownHero;

                if (hero != null)
                {
                    hero.Say(FeatureList.Heroes.OnAbandonedByPlayerSay);
                    hero.StopFollow();
                    hero.ChangeState(HERO_STATES.WaitingAroundForCapture);
                }
            }
        }
    }
