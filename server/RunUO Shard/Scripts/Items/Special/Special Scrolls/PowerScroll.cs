...
namespace Server.Items
{
	public class PowerScroll : SpecialScroll
	{
		...
		public static PowerScroll CreateRandom( int min, int max )
		{
			...
			} while (skillName == SkillName.AnimalTaming || skillName == SkillName.AnimalLore);
		...
		public static PowerScroll CreateRandomNoCraft( int min, int max )
		{
			...
			{
				skillName = Skills[Utility.Random( Skills.Count )];
			} while ( skillName == SkillName.Blacksmith || skillName == SkillName.Tailoring || skillName == SkillName.Carpentry || skillName == SkillName.Inscribe || skillName == SkillName.Alchemy || skillName == SkillName.Tinkering);
		...
		public PowerScroll( SkillName skill, double value ) : base( skill, value )
		{
            ...
            if (Value == 105.0 || skill == Server.SkillName.Blacksmith || skill == Server.SkillName.Tailoring || skill == Server.SkillName.Carpentry || skill == Server.SkillName.Inscribe || skill == Server.SkillName.Alchemy || skill == Server.SkillName.Tinkering )
		...
		public override bool CanUse(Mobile from)
		{
			...
			else if (skill.Value < this.Value - 5)
			{
				// Send message: Your skill is too low to use this power scroll.
				// TODO: convert to localized message
				from.SendMessage(String.Format("Your {0} is too low to use this power scroll.", skill.Name));
				return false;
			}
