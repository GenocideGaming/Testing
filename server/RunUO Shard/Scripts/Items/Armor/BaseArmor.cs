	...
	public abstract class BaseArmor : Item, IScissorable, IFactionItem, ICraftable, IWearableDurability
	{
		...
		public virtual double ArmorRating
		{
			get
			{
                ...
                {
                     ar += FeatureList.ArmorChanges.ArtifactARBonusPerLevel * (int)m_Protection;                 
                }

                ar += -1 + (FeatureList.ArmorChanges.CraftedARBonus * (int)m_Quality);
                
				return ScaleArmorByDurability( ar );
			}
		}

        public virtual double ScaleArmorByDurability(double ar)
        {
            double newAr = ar;

            if (m_MaxHitPoints > 0 && m_HitPoints < m_MaxHitPoints)
            {
                double hits = (double)m_HitPoints;
                double maxhits = (double)m_MaxHitPoints;

                double missingDurability = 1 - (hits / maxhits); //Percentage of hitPoints missing
                double newArmorScalar = 1 - (missingDurability * FeatureList.ArmorChanges.DurablilityImpactScalar); //Impact of missing hitpoints factored in
                
                newAr = Math.Ceiling(ar * newArmorScalar); 
            }
		...
		protected void Invalidate()
		{
            if (Parent is Mobile)
            {
                ((Mobile)Parent).CalculateEquippedArmor();
        ...
        {
            if (Hue == ArmamentDyeHues.Forsaken ||
                Hue == ArmamentDyeHues.Paladin ||
                Hue == ArmamentDyeHues.Urukhai)
            {
                return true;
            }
            else
        ...
        public bool CheckArmamentDyedEquip(Mobile from)
        {
            ...
            if (Hue == ArmamentDyeHues.Forsaken && !MemberOfMilitia("Forsaken", playerMilitiaState))
            {
                from.SendMessage("You are not entitled to wear that because you are not a member of the Forsaken.");
                return false;
            }
            if (Hue == ArmamentDyeHues.Paladin && !MemberOfMilitia("Paladin Order", playerMilitiaState))
            {
                from.SendMessage("You are not entitled to wear that because you are not a member of the Paladin Order.");
                return false;
            }
            if (Hue == ArmamentDyeHues.Urukhai && !MemberOfMilitia("Uruk-hai", playerMilitiaState))
            {
