			...
			if ( attacker.StunReady )
			{
				if ( attacker.CanBeginAction( typeof( Fists ) ) )
				{
					...
					if ( StunSuccessful( attacker ) )
					{
						...
						// store the defender's stamina before stun status takes effect
						int storeDefenderStamina = defender.Stam;

						if(attacker is PlayerMobile && defender is PlayerMobile)
						{
							// for players, we check if they're alive to apply the stun
							// but won't check if they're alive when it's removed because we want to remove the stun regardless of Alive or not
							if(defender != null && !defender.Deleted && defender.Alive)
							{
								...
								(
									...
									delegate
									{
										...
										{
											defender.SendMessage("Your stun has worn off");
											defender.SendSpeedControl(SpeedControlType.Disable);
											defender.Stam = storeDefenderStamina;
										}
									}
								);
							}
						}
						else if(attacker is PlayerMobile && defender is BaseCreature)
						{
							if(defender != null && !defender.Deleted && defender.Alive)
							{
								// this will make it so they can't walk but can still attack, etc
								defender.CantWalk = true; 
						...
						else if(attacker is BaseCreature && defender is PlayerMobile)
						{
							...
							{
								defender.CantWalk = true;
								defender.SendMessage("You have been hamstrung!");

								/*
								Use as metaphor. Literally, to "hamstring" an individual is to sever the tissues of their hamstring. 
								As a metaphor, to be "hamstrung" suggests being limited, by external imposition or not, 
								in a way that prevents full freedom of movement or utilization of resources.
								*/
