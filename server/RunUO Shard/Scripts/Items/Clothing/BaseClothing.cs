...
namespace Server.Items
{
	...
	public abstract class BaseClothing : Item, IDyable, IScissorable, IFactionItem, ICraftable, IWearableDurability
	{
		...
		public int StatCapBonus
		{
			get{ return m_StatCapBonus; }
			set{ m_StatCapBonus = value; InvalidateProperties(); }
		}
		...
		{
            if (IsCitizenshipDyed())
            {
                if (!CheckCitizenshipEquip(from))
                    return false;
            }
            if (IsMilitiaDyed())
            {
                if (!CheckMilitiaEquip(from))
                    return false;
            }
            if (IsVeteranMilitiaDyed())
            {
                if (!CheckVeteranMilitiaEquip(from))
                    return false;
            }
        ...
        private bool CitizenOf(string p, ICommonwealth playerCitizenship)
        {
            if (playerCitizenship == null)
                return false;

            if (playerCitizenship.Definition.TownName == p)
                return true;
            else
                return false;
        }
        ...
        public bool IsCitizenshipDyed()
        {
            if (this.Hue == CitizenDyeHue.Yew ||
                this.Hue == CitizenDyeHue.Trinsic ||
                this.Hue == CitizenDyeHue.Blackrock)
            {
                return true;
            }
            else
        ...
        public bool CheckCitizenshipEquip(Mobile from)
        {
            ICommonwealth playerCitizenship = Commonwealth.Find(from);

            if (Hue == CitizenDyeHue.Yew && !CitizenOf("Yew", playerCitizenship))
            {
                from.SendMessage("You are not entitled to wear that because you are not a citizen of Yew.");
                return false;
            }

            if (Hue == CitizenDyeHue.Trinsic && !CitizenOf("Trinsic", playerCitizenship))
            {
                from.SendMessage("You are not entitled to wear that because you are not a citizen of Trinsic.");
                return false;
            }

            if (Hue == CitizenDyeHue.Blackrock && !CitizenOf("Blackrock", playerCitizenship))
            {
                from.SendMessage("You are not entitled to wear that because you are not a citizen of Blackrock.");
                return false;
            }

            return true;
        }
        public bool IsMilitiaDyed()
        {
            if (this.Hue == MilitiaDyeHue.Forsaken ||
               this.Hue == MilitiaDyeHue.Paladin ||
               this.Hue == MilitiaDyeHue.Urukhai)
            {
                return true;
            }
            else
        ...
        public bool IsVeteranMilitiaDyed()
        {
              if (this.Hue == MilitiaVeteranClothHue.Forsaken ||
               this.Hue == MilitiaVeteranClothHue.Paladin ||
               this.Hue == MilitiaVeteranClothHue.Urukhai)
            {
                return true;
            }
            else
        ...
        public bool MemberOfMilitia(string townName, PlayerState pcs)
        {
            if (pcs == null)
                return false;

            if (pcs.Faction.Definition.FriendlyName == townName)
                return true;
            else
                return false;
        }

        public bool VeteranMemberOfMilitia(string townName, PlayerState pcs)
        {
            if (!MemberOfMilitia(townName, pcs))
                return false;
        ...
        public bool CheckMilitiaEquip(Mobile from)
        {
            ...
            if (Hue == MilitiaDyeHue.Forsaken && !MemberOfMilitia("Forsaken", playerMilitiaState))
            {
                from.SendMessage("You are not entitled to wear that because you are not a member of the Forsaken.");
                return false;
            }
            if (Hue == MilitiaDyeHue.Paladin && !MemberOfMilitia("Paladins", playerMilitiaState))
            {
                from.SendMessage("You are not entitled to wear that because you are not a member of the Paladin Order.");
                return false;
            }
            if (Hue == MilitiaDyeHue.Urukhai && !MemberOfMilitia("Uruk-hai", playerMilitiaState))
            {
                from.SendMessage("You are not entitled to wear that because you are not a member of the Uruk-hai.");
                return false;
            }
        ...
        public bool CheckVeteranMilitiaEquip(Mobile from)
        {
            if (from.AccessLevel > AccessLevel.Player)
                return true;
            ...
            if (Hue == MilitiaVeteranClothHue.Forsaken && !VeteranMemberOfMilitia("Forsaken", playerMilitiaState))
            {
                from.SendMessage("You are not entitled to wear that because you are not a veteran member of the Forsaken.");
                return false;
            }
            if (Hue == MilitiaVeteranClothHue.Paladin && !VeteranMemberOfMilitia("Paladin Order", playerMilitiaState))
            {
                from.SendMessage("You are not entitled to wear that because you are not a veteran member of the Paladin Order.");
                return false;
            }
            if (Hue == MilitiaVeteranClothHue.Urukhai && !VeteranMemberOfMilitia("Uruk-hai", playerMilitiaState))
            {
                from.SendMessage("You are not entitled to wear that because you are not a veteran member of the Uruk-hai.");
                return false;
            }
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			// 6
			writer.Write((int)m_StatCapBonus);
