...
namespace Server.Items
{
	...
	public class Key : Item
	{
		...
		{
			// rely on the Item.cs isAccessibleTo function to determine whether it is reachable or not
		...
		private class UnlockTarget : Target
		{
			...
			{
				if ( m_Key.Deleted ) // || !m_Key.IsChildOf( from.Backpack ) )  // rely on Item.cs isAccessible for this
				{
					from.SendLocalizedMessage( 501661 ); // That key is unreachable.
					return;
				}
