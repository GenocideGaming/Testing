...
namespace Server.Items
{
	...
	public class Corpse : Container, ICarvable
	{
		...
		private void AssignInstancedLoot()
		{
			...
			//deleted this party loot shit here. 
		}
		...
		private static TimeSpan m_DefaultDecayTime = TimeSpan.FromMinutes( 8.0 );
        private static TimeSpan m_DefaultPlayerDecayTime = TimeSpan.FromMinutes(12.0);
		private static TimeSpan m_BoneDecayTime = TimeSpan.FromMinutes( 8.0 );
			...
			: base( 0x2006 )
		{
            ...
            else if (owner is PlayerMobile)
            {
                BeginDecay( m_DefaultPlayerDecayTime );
            }
            else
            {
                BeginDecay( m_DefaultDecayTime );
		...
		public override void OnItemUsed( Mobile from, Item item )
		{
			...
			if ( item != this && IsCriminalAction( from ) )
				from.CriminalAction( true, true ); //we call guards here
		...
		public override void OnItemLifted( Mobile from, Item item )
		{
			...
			if ( item != this && IsCriminalAction( from ) )
				from.CriminalAction( true, true ); //we call the guards here, i think?
		...
		public virtual void Open( Mobile from, bool checkSelfLoot )
		{
			if ( from.AccessLevel > AccessLevel.Player || from.InRange( this.GetWorldLocation(), 2 ) )
			{
				...
				if ( player != null )
				{
                    player.CorpsesInspectedThisSession++;
				}
		...
		public void Carve( Mobile from, Item item )
		{
			...
			else if ( ((Body)Amount).IsHuman && ItemID == 0x2006 )
			{
				...
				{	
					PlayerMobile pm = (PlayerMobile)dead;
					
					// at risk of stat loss, "Kills" refers to long term murders
					// moving SeveredHead to on corpse chop instead of dropping the head into the killer's backpack
					if(pm.ShortTermMurders >= 5 || pm.Kills >= 5)
					{	
						// initialize with a default value
						int reward = pm.PlayerBounty / 2; // copied formula from SeveredHead.AwardSeveredHead method

						// if there's a bounty on the boards, we get that value instead
						/// reference file: "Scripts/Custom/BountySystem/BountyRegistry.cs"
						if (BountyRegistry.Contains(pm))
						{	
							int keyIndex = Array.FindIndex(BountyRegistry.Postings, w => w.Killer == pm);
							reward = BountyRegistry.Postings[keyIndex].Bounty;
						}

						// this shouldn't happen, but in case it does, this is to prevent rewarding a bank check for 0 or negative gold
						if(reward <= 0)
							reward = 100;

						new SeveredHead(pm, pm.Serial).MoveToWorld( Location, Map );
					}
					else
					{
                ...
                PlayerMobile carver = from as PlayerMobile;
                if (carver != null)
                    carver.CorpsesCarvedThisSession++;

				if ( IsCriminalAction( from ) )
					from.CriminalAction( true, false ); //tasteless but no guard call
			}
