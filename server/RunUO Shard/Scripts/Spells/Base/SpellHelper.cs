...
namespace Server.Spells
{
	...
	public class SpellHelper
	{
		...
		public static TimeSpan GetDamageDelayForSpell( Spell sp )
		{
			...
			if (sp.DamageDelay > 0)
				return TimeSpan.FromSeconds(sp.DamageDelay);
			
			return FeatureList.SpellDelays.DefaultDamageDelay;
		}
        ...
        public static void NullifyDefensiveSpells(Mobile m)
        {
            if (!(m is PlayerMobile))
                return;

            PlayerMobile subject = (PlayerMobile)m;

            subject.MeleeDamageAbsorb = 0;                                // Reactive Armor
            subject.MagicDamageAbsorb = 0;                                // Magic reflect
            Spells.Second.ProtectionSpell.Registry.Remove(subject);       // Protection
        }
		...
		public static bool ValidIndirectTarget( Mobile from, Mobile to )
		{
			...
			if( from is BaseCreature )
			{
                ...
                // pseudoseer walls hit all non-team creatures
                if (AITeamList.CheckSameTeam(from, to) == true)
                {
                    return false;
                }
                else
		...
		private static bool[,] m_Rules = new bool[,]
			{
						/*T2A(Fel)		Ilshenar		Dungeons(Tram),	Wind(Fel),	Dungeons(Fel),	Solen(Tram),	Solen(Fel), CrystalCave(Malas),	Gauntlet(Malas),	Gauntlet(Ferry),	Stronghold,		ChampionSpawn, Dungeons(Tokuno[Malas]), LampRoom(Doom),	GuardianRoom(Doom),	Heartwood*/
/* Recall From */		{ false,		true,			true,			true,		true,			false,			false,		false,				false,				false,				true,			false,			true,				false,				false,			false},          
/* Recall To */			{ false,		false,			false,			false,		false,			false,			false,		false,				false,				false,				false,			false,			false,				false,				false,          false},         
/* Gate From */			{ false,		false,			true,			false,		true,			false,			false,		false,				false,				false,				false,			false,			false,				false,				false,			false},          
...
/* Tele To */			{ true,			true,			true,			true,		true,			true,			true,		false,				true,				false,				false, 			true,			true,				true,				true,			false},         
		...
		public static bool CheckTravel( Mobile caster, Map map, Point3D loc, TravelCheckType type )
		{
			...
			if( caster != null && caster.AccessLevel == AccessLevel.Player && caster.Region != null && caster.Region.IsPartOf( typeof( Regions.Jail ) ) )
			{
            ...
            {
                if (Region.Find(loc, map) is MilitiaStronghold)
                    return true;
            }

            if (type == TravelCheckType.RecallTo || type == TravelCheckType.GateTo)
            {
                if (player != null && player.CitizenshipPlayerState != null && player.FactionPlayerState != null)
                {
                    CommonwealthRegion townRegion = Region.Find(loc, map) as CommonwealthRegion;

                    if (townRegion != null && townRegion.MyCommonwealth == player.CitizenshipPlayerState.Commonwealth && !player.FactionPlayerState.Faction.OwningCommonwealth.Hero.IsHome)
                    {
                        caster.SendMessage("You cannot travel directly into your town when your town hero is captured!");
                        return false;
                    }

                    if (townRegion != null && townRegion.MyCommonwealth != player.CitizenshipPlayerState.Commonwealth)
                    {
                        caster.SendMessage("You cannot travel directly into another town's city limits while you are a member of a militia.");
                        return false;
                    }
                }
            ...
            //World wars regions
            if (type == TravelCheckType.RecallTo || type == TravelCheckType.GateTo || type == TravelCheckType.GateFrom || type == TravelCheckType.Mark)
            {
                if (player != null)
                {
                    WorldWarsRegion wwRegion = Region.Find(loc, map) as WorldWarsRegion;
                    if (wwRegion != null)
                    {
                    	SendInvalidMessage( player, type );
                        return false;
                    }

                    StagingAreaRegion stagingRegion = Region.Find(loc, map) as StagingAreaRegion;
		...
		public static bool IsTrammelDungeon( Map map, Point3D loc )
		{
			...
			return (region.IsPartOf(typeof(DungeonRegion)) && region.Map == Map.Trammel);
		}
		...
		*/
		...
		public static bool IsFactionStronghold( Map map, Point3D loc )
		{
			...
			return (Region.Find( loc, map ).IsPartOf( typeof( MilitiaStronghold ) ));
		}
		...
		public static bool IsTrammel(Map map, Point3D loc)
		{
			int x = loc.X, y = loc.Y;

			return (map == Map.Trammel && x >= 5120 && y >= 2304 && x < 6144 && y < 4096);
		}
		...
		{
			if (FeatureList.SpellChanges.AOEAndFieldSpellsInTown)
				return true;
        ...
        public static bool CheckTownEvilSummon(IPoint3D loc, Mobile caster)
        {
            if (loc is Item)
                loc = ((Item)loc).GetWorldLocation();

            return CheckTownEvilSummon(new Point3D(loc), caster);
        }

        public static bool CheckTownEvilSummon(Point3D loc, Mobile caster)
        {
            if (IsTown(loc, caster))
            {
                caster.SendLocalizedMessage(500946); // You cannot cast this in town!
		...
		public static void CheckReflect( int circle, ref Mobile caster, ref Mobile target )
		{
			...
			if( target.MagicDamageAbsorb > 0 )
			{
                ...
                if (target is BaseCreature)
                    ((BaseCreature)target).CheckReflect(caster, ref canReflect);
        ...
        public static void Damage(Spell spell, Mobile target, double damage, bool interruptSpellcasting)
		{
			TimeSpan ts = GetDamageDelayForSpell( spell );

			Damage( spell, ts, target, spell.Caster, damage, interruptSpellcasting );
		}

        public static void Damage(TimeSpan delay, Mobile target, double damage )
        {
            Damage(delay, target, null, damage, true);
        }

		public static void Damage( TimeSpan delay, Mobile target, double damage, bool interruptSpellcasting )
		{
			Damage( delay, target, null, damage, interruptSpellcasting );
		}

		public static void Damage( TimeSpan delay, Mobile target, Mobile from, double damage, bool interruptSpellcasting )
		{
			Damage( null, delay, target, from, damage, interruptSpellcasting );
		}

        public static void Damage(TimeSpan delay, Mobile target, Mobile from, double damage)
        {
            Damage(null, delay, target, from, damage, true);
        }

		public static void Damage( Spell spell, TimeSpan delay, Mobile target, Mobile from, double damage, bool interruptSpellcasting )
		{
            int iDamage = (int)damage;

            //Parry Chance for Damaging Spell
            if (from is PlayerMobile && target is PlayerMobile && FeatureList.ParryChanges.ParryMagicPVP == false)
            {
                //Do Nothing For Now
            }
            ...
            {             
                ...
                if (shield != null)                  
                    iDamage = shield.OnSpellHit(spell, iDamage);                
            }  			
			...
			if( delay == TimeSpan.Zero )
			{
				...
				target.Damage( iDamage, from, true, interruptSpellcasting );
			}

			else
			{
                new SpellDamageTimer( spell, target, from, iDamage, delay, interruptSpellcasting ).Start();
			}
		...
		private class SpellDamageTimer : Timer
		{
            ...
            private bool m_interruptSpellcasting;

			public SpellDamageTimer( Spell s, Mobile target, Mobile from, int damage, TimeSpan delay, bool interruptSpellcasting )
				: base( delay )
			{
				m_Target = target;
				m_From = from;
				m_Damage = damage;
				m_Spell = s;
                m_interruptSpellcasting = interruptSpellcasting;
			...
			protected override void OnTick()
			{
				...
				m_Target.Damage( m_Damage, m_From, true, m_interruptSpellcasting );
        ...
        internal static void CalculateDiminishingReturns(MagerySpell spell, Mobile Caster, Mobile target)
        {
        	if ( (spell is Spells.Second.HarmSpell) )
                target.HarmsTaken += 1;

            if (!(spell is Spells.Second.HarmSpell) && spell.Circle >= SpellCircle.Third)
                target.HarmsTaken = 0;
        }
    }
