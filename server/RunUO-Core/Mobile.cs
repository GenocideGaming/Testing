...
//using Server.Scripts (from Custom/FeatureList)
...
namespace Server
{
    ...
    public class Mobile : IEntity, IHued, IComparable<Mobile>, ISerializable, ISpawnable
    {
        ...
        private Race m_Race;
        private bool mInExileRegion; //whether an exiled mobile is in a region they are exiled from
        private double m_EquipmentArmor;
        private double m_ArmorRating;

        private ArmorDrainTimer m_ArmorDrainTimer;
        private DateTime m_ArmorDrainExpiration;
        private int m_ArmorDrain;

        private SwingAdjustmentTimer m_SwingAdjustmentTimer;
        private DateTime m_SwingAdjustmentExpiration;
        private int m_SwingAdjustment;

        private StealthAttackTimer m_StealthAttackTimer;
        ...
        protected ulong m_TeamFlags = 0x1;
        [CommandProperty(AccessLevel.GameMaster)]
        public virtual ulong TeamFlags
        {
            get
            {
                return m_TeamFlags;
        ...
        public bool InExileRegion { get { return mInExileRegion; } set { mInExileRegion = value; } }
        ...
        protected bool m_IgnoreMobiles;
        [CommandProperty(AccessLevel.GameMaster)]
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public int SwingAdjustment
        {
            get
            {
                return m_SwingAdjustment;
            }
            set
            {
                m_SwingAdjustment = value;
            }
        }
        ...
        public void DoArmorDrain(int amount, int amountMax, double duration)
        {
            m_ArmorDrainTimer = new ArmorDrainTimer(this, TimeSpan.FromSeconds(duration));
            m_ArmorDrainTimer.Start();

            if (m_ArmorDrain + amount > amountMax)
            {
                amount = amountMax - m_ArmorDrain;
            }

            m_ArmorDrain += amount;
            VirtualArmorMod -= amount;

            m_ArmorDrainExpiration = DateTime.Now + TimeSpan.FromSeconds(duration);

            m_ArmorDrainTimer = new ArmorDrainTimer(this, TimeSpan.FromSeconds(duration));
            m_ArmorDrainTimer.Start();
        }

        public void DoSwingAdjustment(int amount, int amountMax, double duration)
        {
            if (m_SwingAdjustment < amountMax)
            {
                m_SwingAdjustment += amount;
            }

            m_SwingAdjustmentExpiration = DateTime.Now + TimeSpan.FromSeconds(duration);

            m_SwingAdjustmentTimer = new SwingAdjustmentTimer(this, TimeSpan.FromSeconds(duration));
            m_SwingAdjustmentTimer.Start();
        }

        public void CancelSwingAdjustment()
        {
            m_SwingAdjustment = 0;
            m_SwingAdjustmentExpiration = DateTime.Now;
        }
        ...
        public void DoStealthAttack(double duration)
        {
            ...
            m_StealthAttackExpiration = DateTime.Now + TimeSpan.FromSeconds(duration);
        ...
        public void CancelStealthAttack(double duration)
        {
            ...
            m_StealthAttackExpiration = DateTime.Now;
        }
        ...
        private class HitsTimer : Timer
        {
            ...
            protected override void OnTick()
            {
                if (m_Owner.CanRegenHits)// m_Owner.Alive && !m_Owner.Poisoned )
                    m_Owner.Hits += m_Owner.RegenHitsPerTick;
        ...
        private class ArmorDrainTimer : Timer
        {
            private Mobile m_Mobile;

            public ArmorDrainTimer(Mobile m, TimeSpan duration)
                : base(duration)
            {
            ...
            protected override void OnTick()
            {
                if (DateTime.Now >= m_Mobile.m_ArmorDrainExpiration)
                {
                    //Remove VirtualArmor Mod Penalty
                    m_Mobile.VirtualArmorMod += m_Mobile.m_ArmorDrain;
                    m_Mobile.m_ArmorDrain = 0;

                    m_Mobile.m_ArmorDrainTimer.Stop();
                }
            }
        }

        private class SwingAdjustmentTimer : Timer
        {
            private Mobile m_Mobile;

            public SwingAdjustmentTimer(Mobile m, TimeSpan duration)
                : base(duration)
            {
            ...
            protected override void OnTick()
            {
                if (DateTime.Now >= m_Mobile.m_SwingAdjustmentExpiration)
                {
                    m_Mobile.m_SwingAdjustment = 0;
                    m_Mobile.m_SwingAdjustmentTimer.Stop();
                }
            }
        }

        private class StealthAttackTimer : Timer
        {
            private Mobile m_Mobile;
            ...
            protected override void OnTick()
            {
                if (DateTime.Now >= m_Mobile.m_StealthAttackExpiration)
                {
        ...
        private class ExpireCriminalTimer : Timer
        {
            ...
            public ExpireCriminalTimer(Mobile m)
                : base(m.InExileRegion ? TimeSpan.FromDays(7) : m_ExpireCriminalDelay) //if we are an exile in a region we are exiled from we need an extremely long criminal timer
            {
                this.Priority = TimerPriority.FiveSeconds;
                m_Mobile = m;
            }
        ...
        public virtual bool FollowersMaxChanged()
        {
            return true;
        }

        public virtual int DetermineFollowersMax()
        {
            int followersMax = 5;
        ...
        {
            //This Only Gets Called if a Mobile is Walked Over (There aren't Many Pure Mobiles: Mostly are BaseCreatures or PlayerMobiles)
        ...
        public virtual void CriminalAction(bool message, bool isGuardCallingAction)
        {
            if (m_Deleted)
                return;
            ...
            this.Region.OnCriminalAction(this, message, isGuardCallingAction);
        }
        ...
        public virtual void CalculateEquippedArmor()
        {
        }

        public virtual void CalculateTotalArmor()
        {
            double ar = 0.0;

            ar = EquipmentArmor + VirtualArmor + VirtualArmorMod;
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public virtual double ArmorRating //Display Property in StatusBar
        {
            get
            {
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public double EquipmentArmor
        {
            get
            {
                return m_EquipmentArmor;
            }

            set
            {
                if (m_EquipmentArmor != value)
                {
                    if (value < 0)
                        m_EquipmentArmor = 0;
                    else
                        m_EquipmentArmor = value;
                }
        ...
        public virtual void OnAfterDelete()
        {
            ...
            if (m_ArmorDrainTimer != null)
                m_ArmorDrainTimer.Stop();
        ...
        // Note: maybe in future this could be replaced with a dictionary of counts, if necessary
        // but for now this will do fine
        private int m_HarmsTaken;

        [CommandProperty(AccessLevel.GameMaster)]
        public int HarmsTaken
        {
            get { return m_HarmsTaken; }
            set { m_HarmsTaken = value; }
        }
        ...
        public virtual void Damage(int amount, Mobile from, bool informMount, bool interruptSpellcasting)
        {
            ...
            if (amount > 0)
            {
                ...
                if (m_Spell != null)
                    m_Spell.OnCasterHurt(interruptSpellcasting);
        ...
        public virtual void Deserialize(GenericReader reader)
        {
            ...
            switch (version)
            {
                case 33:
                    {
                        m_RegenHitsPerTick = reader.ReadInt();
                        goto case 32;
                    }
                case 32:
                    {
                        m_EquipmentArmor = reader.ReadInt();
                        goto case 31;
                    }
                case 31:
                    {
        ...
        public virtual void Serialize(GenericWriter writer)
        {
            ...
            writer.Write((int)m_RegenHitsPerTick);
            writer.Write((int)m_EquipmentArmor);
        ...
        public void CheckNearbyGhostVisibility()
        {
            //Scan Immediate Area for Ghosts and Do ValidateGhostVisibility for Each
            if (this.Map != null)
            {
                IPooledEnumerable scanRange = this.Map.GetClientsInRange(this.Location);

                foreach (NetState state in scanRange)
                {
                    //If Mobile is Dead
                    if (!state.Mobile.Alive)
                    {
                        state.Mobile.ValidateGhostVisibility();
                    }
                }
        ...
        public void ValidateGhostVisibility()
        {
            if (this.m_Map != null)
            {
                Packet p = null;
                ...
                foreach (NetState state in eable)
                {  
                    //Mobile Can't See Target     
                    if (!this.CanSee(state.Mobile))
                    {
                        p = state.Mobile.RemovePacket;
                    ...
                    //Mobile Can See Target
                    else
                    {
                        if (state.StygianAbyss)
                        {
                            this.Send(new MobileIncoming(this, state.Mobile));
                        }
                        else
                        {
                            this.Send(new MobileIncomingOld(this, state.Mobile));
                        }
		...
		{
			return Send(new SpeedControl((int)type));
		}
            ...
            if (m_Map != null && ns != null)
            {
                ...
                foreach (object o in eable)
                {
                    ...
                    else if (o is Mobile)
                    {
                        ...
                        {                            
                            //ns.Send(new MobileIncomingOld(this, m));       // SOURCE OF POISON FASTWALK GLITCH
                            if (ns.StygianAbyss)
                            {
        ...
        {
            if (isCriminal)
                CriminalAction(false, false); //no guards called for beneficial actions
        }
        ...
        {
            if (isCriminal)
                CriminalAction(false, true); //guard whack
        }
        ...
        public int m_RegenHitsPerTick = 1;
        [CommandProperty(AccessLevel.GameMaster)]
        public virtual int RegenHitsPerTick
        {
            get { return m_RegenHitsPerTick; }
            set { m_RegenHitsPerTick = value; }
        }
        ...
        public virtual bool CanSee(Item item)
        {
            ...
            if (item.GhostVisible == false && this.Alive == false)
                return false;
        ...
        public virtual bool CanSee(Mobile m, double distance)
        {
            ...
            if (oldLocation != newLocation)
            {
                ...
                if (map != null)
                {
                    ...
                    foreach (NetState ns in eable)
                    {
                        ...
                        if (ns != m_NetState)
                        {
                            //We Are Not Longer in their Visual Update Range
                            if (!Utility.InUpdateRange(newLocation, ns.Mobile.Location))
                            {
                                if (removeThis == null)
                            ...
                            //Within Visual Update Range Still
                            else
                            {
                                double oldDistance = ns.Mobile.GetDistanceToSqrt(oldLocation);
                                double newDistance = ns.Mobile.GetDistanceToSqrt(newLocation);

                                //They Could Previously See Us But Now Can't (Passive Detect/Tracking Checks)
                                if (ns.Mobile.CanSee(this, oldDistance) && !ns.Mobile.CanSee(this, newDistance) && ns.Mobile != this)
                                {
                                    removeThis = this.RemovePacket;
                                    
                                    if (removeThis != null)
                                    {
                                        ns.Mobile.Send(removeThis);
                                    }
                                }

                                //We Could Previously See Them But Now We Can't (Passive Detect/Tracking Checks)
                                if (this.CanSee(ns.Mobile, oldDistance) && !this.CanSee(ns.Mobile, newDistance) && ns.Mobile != this)
                                {     
                                    removeThis = ns.Mobile.RemovePacket;
                                    
                                    if (removeThis != null)
                                    {
                                        this.Send(removeThis);
                                    }
                    ...
                    if (ourState != null)
                    {
                        ...
                        foreach (object o in eable)
                        {
                            ...
                            else if (o != this && o is Mobile)
                            {
                                ...
                                //To Them: We're Now Within Their Visual Update Range
                                ...
                                //To Us: They're Now Within Our Visual Update Range
                                ...
                                //Already Were In Visual Update Range: But Possibly Hidden Before (Passive Detect/Tracking Checks)
                                if (inOldRange)
                                {                                    
                                    double oldDistance = m.GetDistanceToSqrt(oldLocation);
                                    double newDistance = m.GetDistanceToSqrt(newLocation);

                                    //They Couldn't See Us at Old Location But Can Now
                                    if (m.m_NetState != null && (!m.CanSee(this, oldDistance) && m.CanSee(this, newDistance)))
                                    {
                                        if (m.m_NetState.StygianAbyss)
                                        {
                                            m.m_NetState.Send(new MobileIncoming(m, this));
                                    ...
                                    //We Couldn't See Them at Old Location But Can Now
                                    else if (!CanSee(m, oldDistance) && CanSee(m, newDistance))
                                    {
                                        if (ourState.StygianAbyss)
                                        {
                                            ourState.Send(new MobileIncoming(this, m));
                    ...
                    else
                    {
                        ...
                        {                           
                            ...
                            //Teleport or Previously Wasn't in Update Range
                            ...
                            //They Couldn't See Us at Old Distance But Now Can At New Distance
                            else if (!ns.Mobile.CanSee(this, oldDistance) && ns.Mobile.CanSee(this, newDistance))
                            {
                                if (ns.StygianAbyss)
                                {
                                    ns.Send(new MobileIncoming(ns.Mobile, this));
        ...
        public Mobile()
        {
            m_Region = MapFactory.Internal.DefaultRegion;
            m_Serial = SerialFactory.NewMobile;

            DefaultMobileInit();

            WorldFactory.AddMobile(this);

            Type ourType = this.GetType();
            m_TypeRef = WorldFactory.m_MobileTypes.IndexOf(ourType);
        ...
        public virtual void OnSingleClick(Mobile from)
        {
            ...
            if (hue == 0x022) { // NOTE InsideUO hues are 3 higher than these values
                // the best way i could think of to get this info to the Core was through Title
                // since the Core has no concept of anything in the Scripts folder (including PlayerMobile)
                if (Title == "the Outcast")
                {
                    hue = 0x020; // dark red
                }
                else if (Title == "the Shamed")
                {
                    hue = 0x01f; // pinkish
                }
            }
            ...
            // relocated to PlayerMobile so we can check for title of Dread Lord/Lady before applying Lord/Lady prefix
            /* if (ShowFameTitle && (m_Player || m_Body.IsHuman) && m_Fame >= 10000)
            {    
                prefix = (m_Female ? "Lady" : "Lord");
            } */
